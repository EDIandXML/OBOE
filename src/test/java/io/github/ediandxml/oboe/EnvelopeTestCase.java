...
public class EnvelopeTestCase {
	...
	public void testDelimiters() {
		...
		X12Envelope x = new X12Envelope(EnvelopeFactory.buildEnvelope("x12.envelope", ""));
		x.setDelimiters("abc");
		...
		EDIFACTEnvelope e = new EDIFACTEnvelope(EnvelopeFactory.buildEnvelope("EDIFACT.envelope", ""));
		e.setDelimiters("abc");
		...
		TradacomsEnvelope t = new TradacomsEnvelope(EnvelopeFactory.buildEnvelope("Tradacoms.envelope", ""));
		t.setDelimiters("abc");
		...
		Util.setOBOEProperty(Util.X12_DELIMITERS, "|~:");
		Util.setOBOEProperty(Util.EDIFACT_DELIMITERS, "^&*");
		Util.setOBOEProperty(Util.TRADACOMS_DELIMITERS, "^&*");
	...
	public void testDelimitersInX12() throws IOException {
		...
		try (BufferedReader br = new BufferedReader(new FileReader("testFiles/sample.output.840.1"))) {
			...
			X12DocumentHandler dh = new X12DocumentHandler(new FileReader("testFiles/sample.output.840.1"));
			...
			FileInputStream fis = new FileInputStream("testFiles/sample.output.840.newDelimiters");
	...
	public void testDelimitersInX12WithGrouping() throws IOException {
		...
		try (BufferedReader br = new BufferedReader(new FileReader("testFiles/sample.output.840.5010"))) {
			String instring = null;
			StringBuilder sb = new StringBuilder();
			do {
				instring = br.readLine();
			...
			X12DocumentHandler dh = new X12DocumentHandler(new FileReader("testFiles/sample.output.840.5010"));
			...
			FileInputStream fis = new FileInputStream("testFiles/sample.output.840.newDelimiters.5010");
			try (var br2 = new BufferedReader(new InputStreamReader(fis))) {
				String s2 = br2.readLine();
	...
	public void testDelimitersForISAGroupDelimiter() throws OBOEException, FileNotFoundException, IOException {
		...
		X12DocumentHandler x12DocHandler = new X12DocumentHandler(new FileReader("testFiles/sample.output.840.1"));
