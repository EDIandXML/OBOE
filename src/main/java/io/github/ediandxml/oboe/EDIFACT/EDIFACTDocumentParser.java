...
/**
 * class defining methods for parsing EDI Documents EDIFACT2 dependent Document
 * handlers will register with this class to be notified when specific edi
 * objects are created or finished Unlike the old parser these parsers will not
 * contain the objects, the process of adding objects to owning parents (such as
	...
	public boolean parseDocument(Reader inReader, boolean inValidate) throws OBOEException {
		...
		if (findID.compareTo(EDIFACTEnvelope.idInterchangeHeader) != 0) {
			throw new OBOEException(
					"Segment ID " + findID + " found, was expecting " + EDIFACTEnvelope.idInterchangeHeader + ".");
		}
		...
		while ((findID != null) && (findID.compareTo(EDIFACTFunctionalGroup.idHeader) == 0)) {
			...
			if (iterateOnce) {
				...
				Segment UNG_Functional_Group_Header = fg.createAndAddSegment(EDIFACTFunctionalGroup.idHeader);
				UNG_Functional_Group_Header.setByteOffset(et.getInputByteCount());
			...
			while ((findID != null) && (findID.equals("UNH"))) {
				...
				CompositeElement msgID = (CompositeElement) UNH_Message_Header.getElement("S009");
				...
				CompositeElement interChangeSender = (CompositeElement) UNB_Interchange_Header.getElement("S002");
				CompositeElement interRecipient = (CompositeElement) UNB_Interchange_Header.getElement("S003");
				parsedTransactionSet = TransactionSetFactory.buildTransactionSet(findID, null,
						// use OBOE.properties searchDirective
						msgID.getElement("0052").get() + msgID.getElement("0054").get(), // version
				...
				while ((findID != null) && (findID.equals("UNH"))) {
					...
					if (findID.equals("UNH")) {
						...
						if (iterateOnce) {
							// header.
							et.reportError(EDIFACTFunctionalGroup.idHeader + " appears without "
						...
						et.reportError("Unknown or out of place segment (" + findID + ") byte offset ("
			...
			if (iterateOnce) {
				 ...
				 * " is not defined. Expecting " + EDIFACTFunctionalGroup.idTrailer);
				 */
			} else {
				Segment UNE_Functional_Group_Trailer = fg.createAndAddSegment(EDIFACTFunctionalGroup.idTrailer);
