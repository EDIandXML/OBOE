...
public class EDIFACTEnvelope extends Envelope {
	 ...
	 * UNB and UNZ and creates one emtpy functional group
	 *
	...
	public void parse(Node node) throws OBOEException {
		...
		if (cnode.getNodeName().equals("Service_String")) {
			seg = new Segment(getMyTemplate().getTemplateSegment(idServiceString), this);
			addSegment(seg);
		...
		if (cnode.getNodeName()
			...
			throw new OBOEException("Expected " + getMyTemplate().getTemplateSegment(idInterchangeHeader).getShortName()
		...
		if (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
				.getTemplateSegment(EDIFACTFunctionalGroup.idHeader).getShortName()) != 0) {

			FunctionalGroup functionalGroup = new EDIFACTFunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(),
					this);
			...
			do {
				...
				String version = "";
				tde = null;
				CompositeElement messageVersion = (CompositeElement) getSegment("UNB").getElement("S008");
				...
				CompositeElement interchangeRecipient = (CompositeElement) getSegment("UNB").getElement("S003");

				TransactionSet parsedTransactionSet = TransactionSetFactory.buildTransactionSet(tsID, null, version,
						interchangeRecipient.getElement("0010").get(), interchangeRecipient.getElement("0007").get(),
						testProduction);
				...
				if (i == nl.getLength()) {
					...
					.equals(getMyTemplate().getTemplateSegment(idInterchangeTrailer).getShortName()));

		} else {
			while (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
					.getTemplateSegment(EDIFACTFunctionalGroup.idHeader).getShortName()) == 0) {
				FunctionalGroup functionalGroup = new EDIFACTFunctionalGroup(
						getMyTemplate().getTemplateFunctionalGroup(), this);
				addFunctionalGroup(functionalGroup);
				seg = functionalGroup.createAndAddSegment(EDIFACTFunctionalGroup.idHeader);
				SegmentParser.parse(seg, cnode);
				...
				while (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(EDIFACTFunctionalGroup.idTrailer).getShortName()) != 0) {
				...
				seg = functionalGroup.createAndAddSegment(EDIFACTFunctionalGroup.idTrailer);
				SegmentParser.parse(seg, cnode);
	...
	public void parse(PushbackInputStream pis) throws OBOEException, FileNotFoundException, IOException {
		...
		if (id.compareTo(getMyTemplate().getTemplateSegment(idInterchangeHeader).getID()) != 0) {
			throw new OBOEException(
					"Expected " + getMyTemplate().getTemplateSegment(idInterchangeHeader).getID() + " got " + id);
		}
		...
		if (id.compareTo(getMyTemplate().getTemplateFunctionalGroup()
				.getTemplateSegment(EDIFACTFunctionalGroup.idHeader).getID()) != 0) {
			...
			while (id.compareTo(getMyTemplate().getTemplateSegment(EDIFACTFunctionalGroup.idHeader).getID()) == 0) {
				FunctionalGroup functionalGroup = new EDIFACTFunctionalGroup(
						getMyTemplate().getTemplateFunctionalGroup(), this);
				addFunctionalGroup(functionalGroup);
				seg = new Segment(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(EDIFACTFunctionalGroup.idHeader), functionalGroup);
				SegmentParser.parse(seg, pis);
				if (pis.read(me) != 4) {
					throw new OBOEException("expected data not read");
				}
				...
				while (id
						.compareTo(getMyTemplate().getTemplateSegment(EDIFACTFunctionalGroup.idTrailer).getID()) != 0) {
				...
				seg = new Segment(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(EDIFACTFunctionalGroup.idTrailer), functionalGroup);
				SegmentParser.parse(seg, pis);
				if (pis.read(me) != 4) {
					throw new OBOEException("expected data not read");
				}
		...
		if (id.compareTo(getMyTemplate().getTemplateSegment(idInterchangeTrailer).getID()) != 0) {
			throw new OBOEException("Expecting " + getMyTemplate().getTemplateSegment(idInterchangeTrailer).getID());
		}
	...
	public FunctionalGroup createFunctionalGroup() {
		return new EDIFACTFunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(), this);
	}
	...
	public void setFGCountInTrailer() {
		...
		getInterchange_Trailer().setDataElementValue(2, Integer.toString(getFunctionalGroupCount()));
	}
	...
	public void writeFormattedText(Writer inWriter, Format inFormat) throws OBOEException, IOException {
		...
		if (format == Format.VALID_XML_FORMAT) {
			...
			writer.write("<envelope format=\"EDIFACT\"" + io.github.ediandxml.oboe.util.Util.lineFeed);
	...
	public void validate(DocumentErrors inDErr) throws OBOEException {
		...
		if (seg == null) {
			inDErr.addError(0, "Envelope", "Missing " + idInterchangeHeader + "  Segment", this, "1", this,
		...
		seg = getSegment(idInterchangeTrailer);
		if (seg == null) {
			inDErr.addError(0, "Envelope", "Missing " + idInterchangeTrailer + "  Segment", this, "1", this,
					ERROR_TYPE.Integrity);
		} else {
			seg.validate(inDErr);
		}
	...
	public Segment createService_String() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(EDIFACTEnvelope.idServiceString);
		if (tsg == null) {
			return null;
		}
	...
	public Segment createInterchange_Header() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(EDIFACTEnvelope.idInterchangeHeader);
	...
	public Segment createInterchange_Trailer() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(EDIFACTEnvelope.idInterchangeTrailer);
		if (tsg == null) {
			return null;
		}
		Segment seg = new Segment(tsg, this);
