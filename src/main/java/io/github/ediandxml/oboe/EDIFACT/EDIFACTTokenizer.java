...
public class EDIFACTTokenizer extends Tokenizer implements ITokenizer {
	 ...
	 */
	public EDIFACTTokenizer(Reader inReader, DocumentErrors inDErr) throws OBOEException {
		...
		if ((firstSeg[0] == 'U') && (firstSeg[1] == 'N') && (firstSeg[2] == 'A')) {
			...
			try {
				if (firstSeg[9] == 'U') {
					...
					// combination
					((ReaderTokenizer) transactionSetTokenizer).setSkipChar(firstSeg[9]);
				}
			} catch (IOException ioe) {
				ioe.printStackTrace();
				throw new OBOEException(ioe.getMessage());
		...
		} else // use defaults
		{
			...
			transactionSetTokenizer = new ReaderTokenizer(pbr, Envelope.EDIFACT_SEGMENT_DELIMITER, "?");
