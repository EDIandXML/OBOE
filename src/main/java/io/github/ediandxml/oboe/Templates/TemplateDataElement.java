...
public class TemplateDataElement extends TemplateElement {
	 ...
	 */
	public TemplateDataElement(String inID, String inName, int inPosition, String inType, char inRequired,
			String inDesc, int inMinLength, int inMaxLength, String inShortName, IDListProcessor inIDList,
	...
	public String validate(String inText) {
		...
		if (isUsed() == false) {
			if ((inText != null) || (testText.length() > 0)) {
				return "field at position " + getPosition() + " id=" + getID() + " data element is not used.";
			}
		}

		if (isRequired()) {
			if ((inText == null) || (testText.length() == 0)) {
				return "field at position " + getPosition() + " id=" + getID() + " value required.";
			}
		} else // not required
		if ((inText == null) || (testText.length() == 0)) {
			return null;
		...
		if (inText.length() < getMinLength()) {
			return "field at position " + getPosition() + " id=" + getID() + " field value too short.";
		}

		if ((getType().equals("DT")) && (getMaxLength() == 6) && (testText.length() == 8)) {
			; // let's not do this here
		} else if (testText.length() > getMaxLength()) {
			return "field at position " + getPosition() + " id=" + getID() + " field value too long.";
		}
		...
		for (int i = 0; i < testText.length(); i++) {
			...
			if (((ch >= 'A') && (ch <= 'Z')) || ((ch >= '0') && (ch <= '9')) || (ch == '!') || (ch == '"')
					|| (ch == '&') || (ch == '\'') || (ch == '(') || (ch == ')') || (ch == '*') || (ch == '+')
					|| (ch == '\'') || (ch == '-') || (ch == '.') || (ch == '/') || (ch == ':') || (ch == ';')
					|| (ch == '?') || (ch == '=') || (ch == ' ') || ((ch >= 'a') && (ch <= 'z')) || (ch == '%')
					|| (ch == '~') || (ch == '@') || (ch == '[') || (ch == ']') || (ch == '_') || (ch == '{')
					|| (ch == '}') || (ch == '\\') || (ch == '|') || (ch == '<') || (ch == '>') || (ch == '#')
					|| (ch == '$')) {
				;
			} else {
				return "field at position " + getPosition() + " id=" + getID() + " invalid character.";
			}
		}
	...
	public String getDefault() {
		...
		if (loadFromClassMethod != null) {
			...
			if (pos <= 0) {
				logr.error("Can't find method " + loadFromClassMethod + " for DE " + this.getID() + " in "
	...
	public boolean runValidatingMethod(Element inDE, DocumentErrors inDocErrs) throws OBOEException {
		if (validatingMethod == null) {
			return true;
		}
	...
	public String toString() {
		return getType() + ";" + getID() + ";" + getName() + ";" + getPosition();
	}
