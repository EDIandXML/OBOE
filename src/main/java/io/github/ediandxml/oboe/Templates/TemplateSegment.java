 ...
 * 
 */
public class TemplateSegment implements ITemplate, IContainedObject, ITemplateElementContainer {
	...
	public TemplateSegment(String inId, String inName, int inPosition, String inDescription, int inOccurs,
			char inRequired, String inShortName, boolean inUsed, IContainer inParent) {
		setID(inId);
		setName(inName);
	...
	/**
	 * gets TemplateDE from TemplateDE container with the first ID, there can be
	 * multiple
	 * 
	 *
	 * @return TemplateDE
	 ...
	 * transaction segment. If yes then this incoming segment is the segment. <br>
	...
	/**
	 * looking into the tokenized string we ask the first idde field if the data in
	...
	/**
	 * asking template what parsing id field it doesn't like so that it would create
	 * a prevalidated segment.
	 *
	 */
	...
	public String getValidatingMethod() {
		...
		return validatingMethod.getDeclaringClass().getName() + "." + validatingMethod.getName();
	}
	...
	public boolean runValidatingMethod(Segment inSeg, DocumentErrors inDocErrs) throws OBOEException {
	...
	public String toString() {
		return getID() + " " + getName() + " " + getOccurs() + " " + getRequired() + " " + getPosition() + " "
				+ getParent().getID() + " ";
	}
	...
	public int getLastPosition() {
		return myElementContainer.templateDataElementList.lastEntry().getValue().position;
