 ...
 * 
 */
public class TemplateLoop extends MetaTemplateContainer implements ITemplate, IContainedObject {
	 ...
	 */
	public TemplateLoop(String inID, String inName, int inOccurs, char inRequired, String shortName, boolean inUsed,
	...
	/**
	 * returns true if the template loop can prevalidate an incoming edi transaction
	 * segment. If yes then this segment is the first segment for this loop. It
	 * actually defines the loop. <br>
	...
	/**
	 * looking into the tokenized string we ask the first segment's first idde field
	 * if the data in the same position is one of its codes
	 *
	 * @return boolean
	 */
	...
	public boolean runValidatingMethod(Loop inLoop, DocumentErrors inDocErrs) throws OBOEException {
		if (validatingMethod == null) {
			return true;
		}
