 ...
 * 
 */
public class TemplateCompositeElement extends TemplateElement implements ITemplateElementContainer {
	...
	public TemplateCompositeElement(String inId, String inName, char inRequired, int inPosition, String inDescription,
			String inShortName, IContainedObject inParent, int inOccurs, boolean inUsed) {
		setID(inId);
		setName(inName);
	...
	public void addElement(TemplateElement inTemplateElement) {
		...
		if (inTemplateElement.getOccurs() != 1) {
			throw new OBOEException("Data elements within Composite Elements can only have occur values of 1(one).");
		}
	...
	public boolean runValidatingMethod(CompositeElement inCDE, DocumentErrors inDocErrs) throws OBOEException {
		if (validatingMethod == null) {
			return true;
		}
