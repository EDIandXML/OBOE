...
/**
 * class for Template Tables a general class for the transaction set's heading
 * detail and summary.
 *
 * <p>
 ...
 * 
 */
public class TemplateTable extends MetaTemplateContainer implements ITemplate, IContainedObject {
	/**
	 * String XML tag
	 */
	...
	public boolean doYouWantThisNode(Node node) throws OBOEException {
		...
		for (var conElements : theContainer) {
			if (conElements.getContainerType() != ContainerType.Segment) {
				throw new OBOEException("unexpected type " + conElements.getContainerType());
			}
	...
	/**
	 * returns the table id, since there are no tables id it returns a zero-length
	 * string
	 *
	 * @return String
	 */
	...
	public boolean runValidatingMethod(Table inTable, DocumentErrors inDocErrs) throws OBOEException {
		if (validatingMethod == null) {
			return true;
		}
