...
public class TemplateElementContainer implements ITemplateElementContainer {
	...
	@Override
	public void addElement(TemplateElement inTemplateElement) throws OBOEException {
		if (inTemplateElement.getPosition() < 1) {
			throw new OBOEException("Template Element Container position must be positive.");
		}
		...
		// }
		if (templateDataElementList.containsKey(inTemplateElement.getPosition())) {
			throw new OBOEException("Template Element Container already populated.");
		}
		templateDataElementList.put(inTemplateElement.getPosition(), inTemplateElement);
	...
	public boolean canYouPrevalidate() {
		...
		for (var tce : templateDataElementList.values()) {
			...
			if (tce.IAmATemplateComposite()) {
				...
				TemplateDataElement tcde = (TemplateDataElement) tc.getTemplateElement(1);
	...
	public IDListProcessor getIDListThatPrevalidates() {
		for (var tce : templateDataElementList.values()) {
			...
			if (tce.IAmATemplateComposite()) {
				TemplateCompositeElement tc = (TemplateCompositeElement) tce;
				TemplateDataElement tde = (TemplateDataElement) tc.getTemplateElement(1);
	...
	public boolean isThisYou(ITokenizer inToken) {
		...
		for (var tdeme : templateDataElementList.values()) {
			...
			if (tdeme.IAmATemplateComposite()) {
				...
				tde = (TemplateDataElement) tce.myElementContainer.templateDataElementList.get(1);
				...
				if ((tde.isRequired()) && (tde.getType().equals("ID"))) {
					inToken.resetSegment();
					return tde.getIDList().isCodeValid(inToken.getDataElementAt(tokenPos));
				}
			}
		}
	...
	public void whyNotYou(Tokenizer et, MetaContainer errContainer) {
		for (int i = 0; (i < getContainerSize()) && (i < (et.countDataElements() - 1)); i++) {
			et.getNextDataElement();
			if (templateDataElementList.get(i + 1).IAmATemplateComposite()
					&& (templateDataElementList.get(i + 1).isRequired())) {
				TemplateCompositeElement tce = (TemplateCompositeElement) templateDataElementList.get(i + 1);
				...
				if ((tde.isRequired()) && (tde.getType().equals("ID"))) {
					...
					et.resetSegment();
					et.reportError(
							"ID Field " + who.getID() + " with value of " + s
									+ " is invalid or segment is out of place at byte offset(" + et.getInputByteCount()
									+ ") near segment [" + et.getSegmentPos() + "]",
			...
			TemplateDataElement tde = (TemplateDataElement) templateDataElementList.get(i + 1);
			if (tde.getType().equals("ID")) {
				et.reportError(
						"ID Field " + who.getID() + " with value of " + et.getDataElementAt(i + 1)
								+ " is invalid or segment is out of place at byte offset(" + et.getInputByteCount()
								+ ") near segment [" + et.getSegmentPos() + "]",
						errContainer, "7", this, DocumentErrors.ERROR_TYPE.Integrity);
				et.resetSegment();
		...
		et.reportError(
				"ID Field " + who.getID() + " value can not be found near or at byte offset(" + et.getInputByteCount()
						+ ") near segment [" + et.getSegmentPos() + "]",
				errContainer, "7", this, DocumentErrors.ERROR_TYPE.Integrity);
		et.resetSegment();
		return;
