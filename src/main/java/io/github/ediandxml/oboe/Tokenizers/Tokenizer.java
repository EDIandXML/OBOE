...
public abstract class Tokenizer implements ITokenizer {
	...
	/**
	 * returns the version id in the header segments presently only works for X12
	 * field I11.
	 * 
	 * @return String
	 */
	...
	public IDataTokenizer getNextSegment(MetaContainer inSegContainer) {
		if (currentTokenStringToken.startsWith("UNA")) {
			...
			currentTokenizer = new DataTokenizer(temp, tokenseparatorCharacter, escapeCharacters);
			currentSegID = "UNA";
		...
		if (transactionSetTokenizer.hasMoreElements() && (currentTokenStringToken.length() == 0)) {
			dErr.addError(segmentPos, currentSegID, "Invalid token length", inSegContainer, "4", inSegContainer,
					DocumentErrors.ERROR_TYPE.Integrity);
			return new DataTokenizer("", "", "");
		}

		if (currentTokenStringToken.startsWith("BIN")) {
			currentTokenizer = new BinaryTokenizer(currentTokenStringToken, tokenseparatorCharacter,
					(ReaderTokenizer) transactionSetTokenizer);
		} else {
			currentTokenizer = new DataTokenizer(currentTokenStringToken, tokenseparatorCharacter, escapeCharacters);
		}
	...
	public String getRestOfSegment() {
		...
		return ((DataTokenizer) currentTokenizer).getTokenString().substring(3, 8);
	}
	...
	/**
	 * returns the currently parsed data element. It does not return a data element
	 * object but a value.
	 * 
	...
	/**
	 * returns the position of the current dataelement being tokenized. Value is
	 * relative to zero.
	 * 
	...
	public IDataTokenizer makeSubfieldTokenizer() {
		return new DataTokenizer(currentDataElement, new String(tokenGroups), escapeCharacters);
	}
	...
	public void reportError(String inDescription) {
		dErr.addError(getSegmentPos(), getSegmentID(), inDescription, requestingContainer, "?", null,
				DocumentErrors.ERROR_TYPE.Integrity);
	}
	...
	@Override
	public void reportError(String inDescription, MetaContainer inRequestingContainer, String inCode, Object inObject,
			ERROR_TYPE inType) {
		dErr.addError(getSegmentPos(), getSegmentID(), inDescription, inRequestingContainer, inCode, inObject, inType);
	}
	...
	public void reportError(String inDescription, String inCode) {
		dErr.addError(getSegmentPos(), getSegmentID(), inDescription, requestingContainer, inCode, null,
				DocumentErrors.ERROR_TYPE.Integrity);
	}
