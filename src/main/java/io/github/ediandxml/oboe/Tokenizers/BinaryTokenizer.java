...
/**
 * class to assist in tokenizing input transaction sets OBOE - Open Business
 * Objects for EDI
 */
...
public class BinaryTokenizer implements IDataTokenizer {
	 ...
	 */
	public BinaryTokenizer(String inString, String inTokens, ReaderTokenizer mainTokenizer) throws OBOEException {
		int startPos = 0;
		...
		try {
			...
			nfe.printStackTrace();
			throw new OBOEException("BIN01 field not numeric: " + nfe.getMessage());
		}
		int dif = currentPos - startPos;
		if (len < dif) {
			throw new OBOEException(
					"BIN01 field specifies incorrect length, field value =" + len + " length calculated as =" + dif);
		}
		if (len > dif) {
