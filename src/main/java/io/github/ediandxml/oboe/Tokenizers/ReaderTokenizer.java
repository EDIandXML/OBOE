...
public class ReaderTokenizer implements IDataTokenizer {
	...
	/**
	 * cr/lf used as a segment delimiter can also be lf/cr we want to skip the
	 * second character
	 */
	 ...
	 * @param inEscapeCharacters - list of characters to escape and allow tokens in
	 *                           the text pass an empty string or null if no escape
	 *                           characters used
	 */
	public ReaderTokenizer(Reader inReader, String inTokens, String inEscapeCharacters) {
		currentPos = -1;
		...
		try {
			while (tokenReader.ready()) {
				...
				if (escapeCharacters.indexOf(currentChar) > -1) {
					...
					if ((tokens.indexOf(currentChar) > -1) && (escapeCharacters.indexOf(currentChar) > -1)) {
	...
	public byte[] getMoreData(int inLength) throws OBOEException {
		...
		if (pos != inLength) {
			throw new OBOEException("Binary read length error.  Asked for " + inLength + " read only " + pos);
		}
		...
		if (tokens.indexOf(currentChar) < 0) {
			throw new OBOEException(
					"Binary read error.  Last character not end of segment delimiter found " + currentChar);
		}
