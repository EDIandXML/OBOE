...
public class Segment implements IElementContainer, IContainedObject {
	...
	public boolean validate() {
		...
		if (isUsed() == false) {
			dErr.addError(0, getID(), "Segment Not Used", this.getParent(), "6", this,
	...
	public String getFormattedText(Format format) {
		...
		switch (format) {
		case CSV_FORMAT:
			sbFormattedText.append("Segment," + getID() + ",\"" + getName() + "\"" + Util.lineFeed);
			break;
		case XML_FORMAT:
		...
		switch (format) {
		case PREBUILD_FORMAT:
			sbFormattedText.append(Envelope.PREBUILD_SEGMENT_DELIMITER.charAt(0));
			break;
		case X12_FORMAT:
			sbFormattedText.append(Envelope.X12_SEGMENT_DELIMITER.charAt(0));
			break;
		case EDIFACT_FORMAT:
			sbFormattedText.append(Envelope.EDIFACT_SEGMENT_DELIMITER.charAt(0));
			break;
		case TRADACOMS_FORMAT:
			sbFormattedText.append(Envelope.TRADACOMS_SEGMENT_DELIMITER.charAt(0));
			break;
	...
	/**
	 * returns a data element by its position number, not location within ArrayList
	 *
	 * @return DataElement
	 ...
	 *
	 * @return boolean
	 * @param inPosition int is object in array at this position a composite <br>
	 *                   <b>position is relative to 1.</b>
	 */
	 ...
	 * if mandatory subsegment is part of the collection it will create the first
	 * one
	 */
	@Override
	...
	/**
	 * returns the data element associated with being known as making this segment
	 * unique
	 *
	 * @return DataElement
	 */
	...
	public String toString() {
		return "segment id:" + getID() + " name:" + getName() + " seq:" + getPosition();
