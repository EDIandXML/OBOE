...
/**
 * An abstract base class for template containers in OBOE (Open Business Objects
 * for EDI), managing contained objects such as segments, loops, tables,
 * transaction sets, functional groups, or envelopes. Implements ITemplate for
 * identification, IContainer for containment, and Identifier for ID and XML tag
 * access. Subclasses (e.g., TemplateEnvelope, TemplateFunctionalGroup) define
 * specific container types and behaviors.
 *
 *
 * @since 2025
 * @see ITemplate
 ...
 */
public abstract class MetaTemplateContainer implements ITemplate, IContainer, Identifier {

	/**
	 * A map of container keys to indices in theContainer, ensuring ordered access.
	 */
	...
	/**
	 * Constructs a MetaTemplateContainer with empty key and container collections.
	 */
	...
	/**
	 * Adds a contained object to the container, mapping its key to its index.
	 *
	 * @param container the contained object to add
	 * @throws OBOEException if the container type is not allowed in this container
	 */
	public void addContainer(IContainedObject container) throws OBOEException {
		...
		if (!getContainerType().containsType(container.getContainerType())) {
			throw new OBOEException(String.format("Cannot contain %s in %s", container.getContainerType().name(),
					getContainerType().name()));
		}
		ContainerKey key = new ContainerKey(container.getContainerType(), container.getID());
	 ...
	 */
	public IContainedObject getContainer(ContainerType type, String id) throws OBOEException {
		...
		if (index == null) {
			throw new OBOEException(String.format("Unknown container %s with id %s", type.name(), id));
		}
	...
	/**
	 * Trims unused containers and returns the number removed (to be implemented by
	 * subclasses).
	 *
	...
	public TemplateSegment getTemplateSegment(String id) {
		for (IContainedObject entry : theContainer) {
			if (entry.getContainerType() == ContainerType.Segment && entry.getID().equals(id)) {
	...
	/**
	 * Returns the template segment with the specified ID and primary ID value for
	 * prevalidation.
	 *
	 * @param id        the segment ID
	...
	public TemplateSegment getTemplateSegment(String id, String primaryID) {
		for (IContainedObject entry : theContainer) {
			if (entry.getContainerType() == ContainerType.Segment && entry.getID().equals(id)) {
				...
				if (segment.canYouPrevalidate() && segment.isThisYou(primaryID)) {
					return segment;
				}
			}
		}
	...
	public TemplateLoop getTemplateLoop(String id) {
		for (IContainedObject entry : theContainer) {
			if (entry.getContainerType() == ContainerType.Loop && entry.getID().equals(id)) {
	 ...
	 * @return the TemplateSegment, or null if not found
	 */
	public TemplateSegment getTemplateSegmentByIDAndName(String id, String name) {
		for (IContainedObject entry : theContainer) {
			if (entry.getContainerType() == ContainerType.Segment && entry.getID().equals(id)) {
				TemplateSegment segment = (TemplateSegment) entry;
	...
	/**
	 * Checks if the container or its loops contain a segment with the specified ID.
	 *
	...
	public boolean doYouWantThisSegment(String id) throws OBOEException {
		...
		for (IContainedObject entry : theContainer) {
			if (entry.getContainerType() == ContainerType.Loop) {
				...
				if (!loop.theContainer.isEmpty() && loop.theContainer.get(0).getID().equals(id)) {
					return true;
				}
			} else if (entry.getContainerType() == ContainerType.Segment && entry.getID().equals(id)) {
				return true;
			}
		}
		return false;
	 ...
	 *
	 * @return a string describing the container type and number of contained
	 *         objects
	 */
	@Override
	public String toString() {
		return String.format("MetaTemplateContainer[type=%s, containers=%d]", getContainerType().name(),
				theContainer.size());
	}

	/**
	 * Returns the XML tag for the container (to be implemented by subclasses).
	 *
	 * @return the XML tag
	 */
	@Override
