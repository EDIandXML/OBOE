 ...
 */
public abstract class FunctionalGroup extends MetaContainer implements IContainedObject

{
	...
	public FunctionalGroup(TemplateFunctionalGroup inTFG, IContainedObject inParent, String inHeaderID,
			String inTrailerID) {
		super(inTFG);
		setParent(inParent);
		headerId = inHeaderID;
	 ...
	 */
	public void addTransactionSet(TransactionSet inTransactionSet) throws OBOEException {
	...
	public TransactionSet getTransactionSet(int pos) {
		return (TransactionSet) getContainer(ContainerType.TransactionSet, "", pos);
	}
	...
	public String getFormattedText(Format format) {
		...
		for (var tsStream : getKeysByContainerType(ContainerType.TransactionSet)) {
			for (var eachTS : getAllContainers(ContainerType.TransactionSet, tsStream.ID())) {
	...
	public void validate(DocumentErrors inDErr) {
		...
		if (getTransactionSetCount() == 0) {
			inDErr.addError(0, "FG", "No transaction sets", this, "5", this, DocumentErrors.ERROR_TYPE.Integrity);
		} else {
			for (var tsStream : getKeysByContainerType(ContainerType.TransactionSet)) {
				for (var eachTS : getAllContainers(ContainerType.TransactionSet, tsStream.ID())) {
	 ...
	 * Functional Group's segments built from instance methods so we return a null
	...
	{
		return (TemplateSegment) getMyTemplate().getContainer(ContainerType.Segment, inID);
