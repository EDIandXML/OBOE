...
public class TransactionSet extends MetaContainer implements IContainedObject {
	 ...
	 */
	public TransactionSet(TemplateTransactionSet inTemplateTransactionSet, IContainedObject inParent) {
		...
		setShortDescription(((TemplateTransactionSet) getMyTemplate()).getShortDescription());
		setShortName(((TemplateTransactionSet) getMyTemplate()).getShortName());
		setFunctionalGroup(((TemplateTransactionSet) getMyTemplate()).getFunctionalGroup());

		if (((TemplateTransactionSet) getMyTemplate()).getHeaderTemplateTable() != null) {
			headerTable = new Table(((TemplateTransactionSet) getMyTemplate()).getHeaderTemplateTable(), this);
		}
		if (((TemplateTransactionSet) getMyTemplate()).getDetailTemplateTable() != null) {
			detailTable = new Table(((TemplateTransactionSet) getMyTemplate()).getDetailTemplateTable(), this);
		}
		if (((TemplateTransactionSet) getMyTemplate()).getSummaryTemplateTable() != null) {
			summaryTable = new Table(((TemplateTransactionSet) getMyTemplate()).getSummaryTemplateTable(), this);
		}
	}
	...
	/**
	 * continues to parse an EDI Document after an error. Searches through the
	 * containers until last one container to make a good request is found. So the
	...
	public void parse(PushbackInputStream pis) throws OBOEException, IOException {
		...
		if (headerTable != null) {
			idLen = headerTable.getMyTemplate().getContainer().get(0).getID().length();
			...
			pis.unread(me);
			while (headerTable.getMyTemplate().doYouWantThisSegment(id) == true) {
		...
		if (detailTable != null) {
			idLen = detailTable.getMyTemplate().getContainer().get(0).getID().length();
			...
			String id = Util.rightTrim(new String(me));
			pis.unread(me);
			while (detailTable.getMyTemplate().doYouWantThisSegment(id) == true) {
		...
		if (summaryTable != null) {
			idLen = summaryTable.getMyTemplate().getContainer().get(0).getID().length();
			byte me[] = new byte[idLen];
			if (pis.read(me) != idLen) {
				throw new OBOEException("expected data not read");
			}
			String id = Util.rightTrim(new String(me));
			pis.unread(me);
			while (summaryTable.getMyTemplate().doYouWantThisSegment(id) == true) {
	...
	public void setFormat(Format inFormat) {
		if ((inFormat == Format.XML_FORMAT) || (inFormat == Format.X12_FORMAT) || (inFormat == Format.EDIFACT_FORMAT)
				|| (inFormat == Format.VALID_XML_FORMAT) || (inFormat == Format.TRADACOMS_FORMAT)
				|| (inFormat == Format.ACH_FORMAT)) {
			format = inFormat;
		} else {
			if (inFormat != Format.UNDEFINED) {
				logr.error("Invalid transaction set format " + inFormat + ".  Value set to undefined");
				Throwable t = new Throwable();
				StackTraceElement[] es = t.getStackTrace();
				for (StackTraceElement e : es) {
					logr.info(" in class:" + e.getClassName() + " in source file:" + e.getFileName() + " in method:"
	...
	public boolean validate() throws OBOEException {
		...
		if (typeX12) {
			...
			if ((de1 == null) || (de2 == null)) {
				throw new OBOEException("missing data element 329 in either ST or SE");
			}
		...
		if (typeX12 && (trailerSegment != null)) {
			...
			int saidCount = Integer.parseInt(trailerSegment.getElement("96").get());
	...
	public void validate(DocumentErrors inDErr) throws OBOEException {
		...
		if (typeX12) {
			if ((headerSegment != null) && (trailerSegment != null)) {
				Element de1 = headerSegment.getElement("329");
				if (de1 == null) {
					inDErr.addError(0, "ST", "Control Number Mismatch (329)", headerTable, "3", headerSegment,
		...
		if (typeX12 && (trailerSegment != null)) {
			...
			int saidCount;
			try {
				saidCount = Integer.parseInt(trailerSegment.getElement("96").get());
			...
			int readCount = getSegmentCount();
			if (saidCount != readCount) {
				inDErr.addError(0, "96", "Segment Count Mismatch.  Should Be " + readCount, this, "7",
						trailerSegment.getElement("96"), DocumentErrors.ERROR_TYPE.Integrity);
			}
		}
	...
	public String getFormattedText(Format format) {
		...
		switch (format) {
		case XML_FORMAT:
			sbFormattedText.append('<' + getShortName() + " code=\"" + getID() + "\">" + Util.lineFeed);
			break;
		...
		case VALID_XML_FORMAT_WITH_POSITION:
			...
			/*
			 * if (format == Format.EDIFACT_FORMAT) { Segment unh = this.getSegment("UNH");
			 * String v = unh.getCompositeElement("S009").getDataElement("0052").get().trim
		...
		default:
			sbFormattedText.append("Transaction Set: " + getName() + Util.lineFeed);
		}
		if (headerTable != null) {
	...
	/**
	 * like getFormattedText; writes to a Writer object instead of building a
	 * string.
	 *
	...
	public void writeFormattedText(Writer inWriter, Format format) throws IOException {

		switch (format) {
		case XML_FORMAT:
			inWriter.write('<' + getShortName() + " code=\"" + getID() + "\">" + Util.lineFeed);
			break;
		case VALID_XML_FORMAT:
		case VALID_XML_FORMAT_WITH_POSITION:
	...
	public void setTrailerFields() {
		...
		if (typeX12) {
			trailerSegment.setDataElementValue("329", headerSegment.getElement("329").get());
			trailerSegment.setDataElementValue("96", Integer.toString(getSegmentCount()));
		} else {
			trailerSegment.setDataElementValue("0062", headerSegment.getElement("0062").get());
			trailerSegment.setDataElementValue("0074", Integer.toString(getSegmentCount()));
		}
	}
