...
/**
 * Defines the types of containers in OBOE (Open Business Objects for EDI),
 * representing hierarchical structures in EDI documents, such as envelopes,
 * segments, and data elements. Each type has a rank for sorting and specifies
 * which other types it can contain.
 *
 *
 * @since 2025
 * @see ContainerKey
...
public enum ContainerType {
	/**
	 * Represents an individual data element within a segment or composite element.
	 */
	DataElement(7),

	/**
	 * Represents a composite element, grouping multiple data elements within a
	 * segment.
	 */
	CompositeElement(6, "DataElement"),

	/**
	 * Represents an EDI segment, containing data elements or composite elements.
	 */
	Segment(5, "DataElement", "CompositeElement"),

	/**
	 * Represents a loop, grouping segments or other loops (self-referential).
	 */
	Loop(4, "Segment", "Loop"),

	/**
	 * Represents a table, grouping segments or loops within a transaction set.
	 */
	...
	/**
	 * Represents a functional group, containing segments or transaction sets.
	 */
	FunctionalGroup(1, "Segment", "TransactionSet"),

	/**
	 * Represents an envelope, the top-level container for functional groups or
	 * segments.
	 */
	...
	/**
	 * Constructs a ContainerType with the specified rank and contained type names.
	 *
	 * @param rank               the sorting rank (lower is higher priority)
	 * @param containedTypeNames the names of types this container can contain
	 */
			...
			for (String typeName : type.containedTypeNames) {
				try {
					...
					throw new IllegalStateException(
							String.format("Invalid contained type name '%s' for %s", typeName, type), e);
				}
			}
	...
	/**
	 * Returns the rank of this container type, used for sorting in ContainerKey.
	 *
	 ...
	 * @return true if this type can contain the specified type, false otherwise
