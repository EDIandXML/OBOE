...
public abstract class Envelope extends MetaContainer implements IContainedObject {
	...
	/**
	 * response envelope if this is outgoing then it's the receiver response if it's
	 * not outgoing then it is response to this incoming envelope
	 */
	 ...
	 * deliveredPendingReply to current Calendar and class setLastProcessDate
	 */
	 ...
	 * properties file. <br>
	 * if you need to use different delimiters, call this method prior to the first
	 * getFormattedText call. You only need to call it once since all delimiters are
	 * defined as class variables and as such the method contains logic so that it
	 * only works on the first method call. Uses property x12Delimiters.
	 * x12Delimiters format is 3 characters
	 * <ol>
	 * <li>segment
	 * <li>field
	 * <li>group
	 ...
	 * &nbsp; EDIFACTDelimiters="..."
	 * <p>
	 * at this time no exceptions are thrown, all exceptions are caught and written
	 * to the System.err file.
	 */
	...
	public void setDelimiters(String inDelimiters) {
		...
		if ((inDelimiters.length() != 3) && (inDelimiters.length() != 4) && (inDelimiters.length() != 5)) {
			logr.error("set delimiters (" + inDelimiters + ") length (" + inDelimiters.length()
	...
	@Override
	public abstract void parse(PushbackInputStream pis) throws OBOEException, FileNotFoundException, IOException;
	...
	public abstract void writeFormattedText(Writer inWriter, Format format) throws OBOEException, IOException;
	...
	/**
	 * gets the count of Functional Group object in the ArrayList (container);
	 *
	 * @return int count
	 */
	...
	/**
	 * gets a Functional Group object from the ArrayList (container); check for
	...
	public static Format getFormat(Node node) throws OBOEException {
		...
		default:
			throw new OBOEException("Incorrect Interchange Header Node, what is " + firstNodeName);
		}
	 ...
	 * Envelope's segments built from instance methods so we return a null and hope
	 * the call can figure it out
	 *
	...
	public static Envelope processEDIEnvelope(InputStream inStream) throws OBOEException, IOException {
		...
		case 'S':
			pbis.unread(me);
			env = new TradacomsEnvelope(EnvelopeFactory.buildEnvelope("tradacoms.envelope", "notSetYet"));
			env.parse(pbis);
			return env;
		case '1':
			...
			if (me2[0] == '5') {
				...
				env = new ACHEnvelope(EnvelopeFactory.buildEnvelope("ach." + transID + ".envelope", "notSetYet"));
				env.parse(pbis);
				return env;
			}
	...
	public String toString() {
		return "envelope  format:" + getFormat() + " id:" + getID() + " rev:" + getRevision();
		...
		public void run() {
			try {
				...
				while ((i = r.read()) > -1) {
					...
					{
						if ((c == segmentDelimiter.charAt(0)) || (c == groupDelimiter.charAt(0))
								|| (c == fieldDelimiter.charAt(0)) || (c == escapeCharacter.charAt(0))
								|| ((repeatDelimiter.length() > 0) && (c == repeatDelimiter.charAt(0)))) {
					...
					if (c == Envelope.PREBUILD_SEGMENT_DELIMITER.charAt(0)) {
						...
						escTest = true;
					} else if (c == Envelope.PREBUILD_GROUP_DELIMITER.charAt(0)) {
						w.write(groupDelimiter.charAt(0));
					} else if (c == Envelope.PREBUILD_FIELD_DELIMITER.charAt(0)) {
						w.write(fieldDelimiter.charAt(0));
					} else if ((escapeCharacter.length() > 0) && (c == escapeCharacter.charAt(0))) {
						w.write(repeatDelimiter.charAt(0));
					} else if ((repeatDelimiter.length() > 0) && (c == Envelope.PREBUILD_REPEAT_DELIMITER.charAt(0))) {
						w.write(repeatDelimiter.charAt(0));
					} else {
