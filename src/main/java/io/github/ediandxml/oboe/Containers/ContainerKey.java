...
/**
 * A record representing a key for identifying containers in OBOE (Open Business
 * Objects for EDI). Combines a ContainerType and an ID to uniquely identify
 * containers, such as envelopes, segments, or functional groups. Implements
 * Comparable to enable sorting by ContainerType rank and then ID.
 *
 *
 ...
 * @see ContainerType
 */
public record ContainerKey(ContainerType type, String ID) implements Comparable<ContainerKey> {
	...
	/**
	 * Compares this ContainerKey to another based on ContainerType rank and ID.
	 * Containers are first sorted by the rank of their type (ascending), and if
	 * ranks are equal, by their ID (lexicographically).
	 *
	...
	public int compareTo(ContainerKey other) {
		...
		int rankComparison = Integer.compare(type.getRank(), other.type.getRank());
