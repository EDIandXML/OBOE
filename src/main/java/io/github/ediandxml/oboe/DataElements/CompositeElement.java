...
public class CompositeElement extends Element implements IElementContainer, IContainedObject {
	/** log4j object */
	 ...
	 */
	public CompositeElement(TemplateCompositeElement inTemplateComposite, IContainedObject inParent) {
	...
	public String getFormattedText(Format format) {
		...
		if (groupOfMyElements.size() == 0) {
			if ((format == Format.X12_FORMAT) || (format == Format.EDIFACT_FORMAT)
		...
		for (var ec : groupOfMyElements) {
			...
			switch (format) {
			case CSV_FORMAT:
				sbFormattedText.append("Composite DE," + getID() + "," + getName() + "\"" + Util.lineFeed);
				break;
			case XML_FORMAT:
			...
			case VALID_XML_FORMAT_WITH_POSITION:
				...
				if (format == Format.VALID_XML_FORMAT_WITH_POSITION) {
					sbFormattedText.append(" docPosition=\"" + this.getPosition() + "\"");
				}
				sbFormattedText.append(">" + Util.lineFeed);
				break;
			...
			case XML_FORMAT:
				if (getShortName() != null) {
					...
					sbFormattedText.append("</" + getID() + ">" + Util.lineFeed);
				}
				break;
			case VALID_XML_FORMAT:
			case VALID_XML_FORMAT_WITH_POSITION:
			...
			if (getOccurs() > 1) {
				...
				switch (format) {
				case PREBUILD_FORMAT:
					sbFormattedText.append(Envelope.PREBUILD_REPEAT_DELIMITER.charAt(0));
					break;
				case X12_FORMAT:
					if (Envelope.X12_REPEAT_DELIMITER.charAt(0) != '\u0000') {
						sbFormattedText.append(Envelope.X12_REPEAT_DELIMITER.charAt(0));
					}
					break;
				case EDIFACT_FORMAT:
					sbFormattedText.append(Envelope.EDIFACT_REPEAT_DELIMITER.charAt(0));
					break;
				case TRADACOMS_FORMAT:
					sbFormattedText.append(Envelope.TRADACOMS_REPEAT_DELIMITER.charAt(0));
					break;
				default:
					break;
				}
	...
	private ElementContainer createNewElementContainer() {
		currentElementContainer = new ElementContainer((ITemplateElementContainer) myTemplate, this);
	...
	public boolean validate(DocumentErrors inDErr) {
		...
		if (isUsed() == false) {
			inDErr.addError(0, getID(), "Segment Not Used", this.getParent(), "6", this,
	...
	public String toString() {
		return "composite  id:" + getID() + " name:" + getName() + " seq: " + getPosition();
	...
	@Override
	public Element setDataElementValue(String ID, String inValue) throws OBOEException {
		return currentElementContainer.setDataElementValue(ID, inValue);
	}

	@Override
	public Element setDataElementValue(String ID, int offset, String inValue) throws OBOEException {
		return currentElementContainer.setDataElementValue(ID, offset, inValue);
	}

	@Override
	public Element setDataElementValue(int pos, String inValue) throws OBOEException {
