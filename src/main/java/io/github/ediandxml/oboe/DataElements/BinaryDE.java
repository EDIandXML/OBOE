...
public class BinaryDE extends DataElement {
	...
	public String getFormattedText(Format format) {
		...
		switch (format) {
		case CSV_FORMAT:
			sb.append("Binary DE," + getID() + ",\"" + getName() + "," + got + '"' + Util.lineFeed);
			break;
		case XML_FORMAT:
			sb.append('<' + getShortName() + '>' + Util.normalize(got) + "</" + getShortName() + ">" + Util.lineFeed);
			break;
	...
	public String validate(String inText) {
		...
		if (mat.find() == false) {
			return "BINary field: " + getName() + " at position " + getPosition()
	...
	public boolean validate(DocumentErrors inDErr) {
		if (isUsed() == false) {
			if ((get() != null) && (get().length() > 0)) {
				inDErr.addError(0, getID(), "BIN field is not used, see " + getName() + " at position " + getPosition(),
						getParent(), "10", this, DocumentErrors.ERROR_TYPE.Requirement);
				return false;
			} else {
				return true;
			}
