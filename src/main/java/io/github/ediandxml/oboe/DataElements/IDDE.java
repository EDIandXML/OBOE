...
public class IDDE extends DataElement implements IContainedObject {
	...
	public boolean isCodeValid(String testCode) {
		...
		return ((TemplateDataElement) myTemplate).getIDList().isCodeValid(testCode);
	}
	...
	public String describe(String testCode) {
		...
		return ((TemplateDataElement) myTemplate).getIDList().describe(testCode);
	}
	...
	public String getCode(String testDescription) {
		...
		return ((TemplateDataElement) myTemplate).getIDList().getCode(testDescription);
	}
	...
	public String getFormattedText(Format format) {
		...
		for (int ii = 0; ii < value.length; ii++) {
			...
			switch (format) {
			case CSV_FORMAT:
				sb.append("ID DE," + getID() + ",\"" + getName() + "\"," + got + ",\"" + describe(got) + "\""
						+ Util.lineFeed);
				break;
			case XML_FORMAT:
				sb.append("<" + getShortName());

				sb.append(">" + Util.normalize(got) + "</" + getShortName() + ">" + Util.lineFeed);
				break;
			...
			case VALID_XML_FORMAT_WITH_POSITION:
				...
				sb.append(">");
				sb.append("<value description=\"" + Util.normalize(describe(got)) + "\">" + Util.normalize(got)
			...
			case EDIFACT_FORMAT:
				for (int i = 0; i < got.length(); i++) {
					if ((got.charAt(i) == Envelope.EDIFACT_FIELD_DELIMITER.charAt(0))
							|| (got.charAt(i) == Envelope.EDIFACT_GROUP_DELIMITER.charAt(0))
							|| (got.charAt(i) == Envelope.EDIFACT_SEGMENT_DELIMITER.charAt(0))
							|| (got.charAt(i) == Envelope.EDIFACT_ESCAPE_CHARACTER.charAt(0))
							|| (got.charAt(i) == Envelope.EDIFACT_REPEAT_DELIMITER.charAt(0))) {
						sb.append(Envelope.EDIFACT_ESCAPE_CHARACTER.charAt(0) + got.charAt(i));
					} else {
			...
			case TRADACOMS_FORMAT:
				for (int i = 0; i < got.length(); i++) {
					if ((got.charAt(i) == Envelope.TRADACOMS_FIELD_DELIMITER.charAt(0))
							|| (got.charAt(i) == Envelope.TRADACOMS_GROUP_DELIMITER.charAt(0))
							|| (got.charAt(i) == Envelope.TRADACOMS_SEGMENT_DELIMITER.charAt(0))
							|| (got.charAt(i) == Envelope.TRADACOMS_ESCAPE_CHARACTER.charAt(0))
							|| (got.charAt(i) == Envelope.TRADACOMS_REPEAT_DELIMITER.charAt(0))) {
						sb.append(Envelope.TRADACOMS_ESCAPE_CHARACTER.charAt(0) + got.charAt(i));
					} else {
						sb.append(got.charAt(i));
					}
				}
				...
				break;
			default:
				sb.append("\t" + getName() + ": " + describe(got) + " : " + got + Util.lineFeed);
				break;
			}
		}
		return sb.toString();
	...
	public boolean validate(DocumentErrors inDErr) {
		...
		if (isCodeValid(value[0]) == false) {
			inDErr.addError(
					getPosition(), getID(), "Invalid ID dataelement text (" + value[0] + "), see " + getName()
							+ " at position " + getPosition(),
					getParent(), "7", this, DocumentErrors.ERROR_TYPE.Integrity);
			return false;
		}
