...
public class RealDE extends DataElement {
	...
	/**
	 * sets the fields contents by moving cursor, cursor will wrap around; not
	 * formatted
	 *
	 * @param inValue String contents
	 */
	...
	public void setNext(String inValue) throws OBOEException {
		...
		for (int i = 0; i < inValue.length(); i++) {
			...
			if (inValue.charAt(i) == '.') {
				if (foundDecimal) {
					throw new OBOEException("numeric parsing error " + inValue + " contains multiple decimal points");
				}
				foundDecimal = true;
	...
	public String get(int inPos) {
		...
		if ((l < getMinLength()) || ((l == getMinLength()) && (value[inPos].indexOf('.') > -1))) // ignore
		// decimal
		// point
		{
		...
		if (Util.propertyFileIndicatesRealNumbersRetainPrecision()) {
			...
			while ((sb.length() < getMaxLength()) && (currentPrecision < savePrecision[inPos])) {
				sb.append("0");
				currentPrecision++;
			}
	...
	public String getFormattedText(Format format) {
		...
		for (i = 0; i < value.length; i++) {
			...
			case X12_FORMAT:
				sb.append(got);
				if ((getOccurs() > 1) && (i < repeatCnt) && (Envelope.X12_REPEAT_DELIMITER.charAt(0) != '\u0000')) {
