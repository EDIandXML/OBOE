...
/**
 * class for Data Elements defined as Numeric (Nx - where x is an integer
 * representing number of decimal places)
 *
 * 
 */
...
public class NumericDE extends DataElement implements IContainedObject {
	 ...
	 *
	 * @param inValue String contents
	 * @exception OBOEException inValue contains format characters if field defined
	 *                          with type of N#
	 *
	 */
	@Override
	public void setNext(String inValue) throws OBOEException {
		...
		if (inValue.length() < getMinLength()) {
			preformattedError = "field at position " + (cursor + 1) + " id=" + getID() + " field value too short.";

		}

		if (inValue.length() > getMaxLength()) {
			preformattedError = "field at position " + (cursor + 1) + " id=" + getID() + " field value too long.";
		}
		if (cursor >= getOccurs()) {
			cursor = 0;
		}
		...
		for (i = 0; i < inValue.length(); i++) {
			...
			if (inValue.charAt(i) == dfs.getDecimalSeparator()) {
				if (fractionDigits == -1) {
					...
					throw new OBOEException(getID() + ": value contains decimal separator character("
							+ inValue.charAt(i) + "), use setFormatted method");
				}
			}
			if (Character.isDigit(inValue.charAt(i)) == false) {
				if (fractionDigits > -1) {
					throw new OBOEException(getID() + ": value " + inValue + " contains format character("
							+ inValue.charAt(i) + "), use setFormatted method");
				}
			}
		}
	 ...
	 *
	 * @return int returns length of set value, can have a null exception if value
	 *         is not set.
	 *
	 */
	@Override
	...
	public String get(int inPos) {
		...
		for (; i < stop; i++) {
			if ((value[inPos].charAt(i) > '0') && (value[inPos].charAt(i) <= '9')) {
				break;
			}
		}
	...
	public String getFormattedText(Format format) {
		...
		for (int ii = 0; ii < value.length; ii++) {
			...
			if ((fractionDigits > -1) && (got.length() > 0)) {
				...
				if (fractionDigits > 0) {
					nf.setMinimumIntegerDigits(getMinLength() - fractionDigits - 1);
					// subtract 1 for decimal
					nf.setMaximumIntegerDigits(getMaxLength() - fractionDigits - 1);
				} else {
			...
			case X12_FORMAT:
				...
				sb.append(got);
				if ((getOccurs() > 1) && (ii < repeatCnt) && (Envelope.X12_REPEAT_DELIMITER.charAt(0) != '\u0000')) {
			...
			case XML_FORMAT:
				...
				sb.append(">" + got + "</" + getShortName() + ">" + Util.lineFeed);
				break;
			...
			case VALID_XML_FORMAT_WITH_POSITION:
				...
				sb.append(">");
				sb.append("<value>" + got + "</value></element>" + Util.lineFeed);
				break;

			case CSV_FORMAT:
				sb.append("DE," + getID() + ",\"" + getName() + "\",\"" + got + "\"" + Util.lineFeed);
				break;
			default:
	...
	public String setFormatted(String input) throws OBOEException {
			...
			if (Character.isDigit(input.charAt(i))) {
				...
				if (negative) {
					throw new OBOEException(
							getID() + ":invalid character at " + i + " for " + input + " second negative ");
				}
				if (nodigityet == false) {
					throw new OBOEException(
							getID() + ":invalid character at " + i + " for " + input + "  negative after number");
				}
				if (nowdecimal) {
					throw new OBOEException(
							getID() + ":invalid character at " + i + " for " + input + " negative after decimal");
				}
				negative = true;
			} else if (input.charAt(i) == dfs.getDecimalSeparator()) {
				if (nowdecimal) {
					throw new OBOEException(
							getID() + ":invalid character at " + i + " for " + input + " second decimal");
				}
				nowdecimal = true;
			} else if (input.charAt(i) == dfs.getGroupingSeparator()) {
				if (nowdecimal) {
					throw new OBOEException(getID() + ":invalid character at " + i + " for " + input
				...
				if (!Character.isDigit(input.charAt(i + 1)) || !Character.isDigit(input.charAt(i + 2))) {
					...
					throw new OBOEException(input.charAt(i) + " invalid character at " + i + " for " + input
							+ " not all digits after group separator");
				}
				if (((i + 4) < input.length()) && Character.isDigit(input.charAt(i + 4))) {
					throw new OBOEException(getID() + ":invalid character at " + i + " for " + input
		...
		/*
		 * if (negative) { for (i = 1; i<getMaxLength(); i++) { if (outn[i] != '0') {
		 * outn[0] = dfs.getMinusSign(); break; } }
		 */
	...
	public String validate(String inText) {
		...
		String returnMessage = ((TemplateDataElement) myTemplate).validate(inText);
	...
	public boolean validate(DocumentErrors inDErr) {
		...
		if (preformattedError.length() > 0) {
			inDErr.addError(0, getID(), preformattedError, getParent(), "10", this,
		...
		if (isUsed() == false) {
			if ((value[0] != null) && (value[0].length() > 0)) {
				inDErr.addError(0, getID(), "field is not used, see " + getName() + " at position " + getPosition(),
						getParent(), "10", this, DocumentErrors.ERROR_TYPE.Integrity);
				return false;
			} else {
				return true;
			}
		...
		if (isRequired()) {
			if ((value[0] == null) || (value[0].length() == 0)) {
				inDErr.addError(0, getID(), "value[0] Required, see " + getName() + " at position " + getPosition(),
						getParent(), "1", this, DocumentErrors.ERROR_TYPE.Integrity);
				return false;
			}
		} else // not required
		if ((value[0] == null) || (value[0].length() == 0)) {
		...
		if (value[0].length() < getMinLength()) {
			inDErr.addError(
					getPosition(), getID(), "Data element value (" + value[0] + ") Too Short, see " + getName()
							+ " at position " + getPosition(),
					getParent(), "4", this, DocumentErrors.ERROR_TYPE.Integrity);
			return false;
		}

		if ((value[0].charAt(0) == '-') && ((value[0].length() - 1) == getMaxLength())) {
			;
		} else if (value[0].length() > getMaxLength()) {
			inDErr.addError(
					getPosition(), getID(), "Data element value (" + value[0] + ") Too Long, see " + getName()
							+ " at position " + getPosition(),
					getParent(), "5", this, DocumentErrors.ERROR_TYPE.Integrity);
			return false;
		}

		if (myTemplate != null) {
			return ((TemplateDataElement) myTemplate).runValidatingMethod(this, inDErr);
		}
