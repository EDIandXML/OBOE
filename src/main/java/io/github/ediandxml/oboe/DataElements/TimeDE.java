...
public class TimeDE extends DataElement implements IContainedObject {
	...
	/**
	 * sets the value (sets cursor to zero) stored using an input string in the
	 * format HHMMSS where
	 * <UL>
	 * <LI>HH is hour
	 * <LI>MM is minutes
	...
	/**
	 * sets the value (by moving cursor, cursor will wrap around) stored using an
	 * input string in the format HHMMSS where
	 * <UL>
	 * <LI>HH is hour
	 * <LI>MM is minutes
	 * <LI>SS is seconds,
	...
	public void setNext(String inTime) {
		...
		calendar.set(Calendar.HOUR_OF_DAY, Integer.parseInt(inTime.substring(0, 2)));
		...
		if (inTime.length() > 5) {
			calendar.set(Calendar.SECOND, Integer.parseInt(inTime.substring(4, 6)));
		}

		if (inTime.length() < 8) {
			calendar.set(Calendar.MILLISECOND, 0);
		} else {
			calendar.set(Calendar.MILLISECOND, Integer.parseInt(inTime.substring(6, 8)));
		}
	...
	public String getFormattedText(Format format) {
		...
		for (int ii = 0; ii < value.length; ii++) {
			...
			switch (format) {
			case CSV_FORMAT:
				sb.append("Time DE," + getID() + ",\"" + getName() + "\",\"" + sdf.format(calendar.getTime()) + "\""
						+ Util.lineFeed);
				break;
			case XML_FORMAT:
				sb.append("<" + getShortName());
				sb.append(">" + sdf.format(calendar.getTime()) + "</" + getShortName() + ">" + Util.lineFeed);
				break;
			...
			case TRADACOMS_FORMAT:
				...
				break;
			default:
				sb.append("\t" + getName() + ": " + sdf.format(calendar.getTime()) + Util.lineFeed);
			}
		}
		return sb.toString();
	}
	...
	public String validate(String inText) {
		...
		if ((inText.length() == 4) || (inText.length() == 6) || (inText.length() == 7) || (inText.length() == 8)) {
			;
		} else {
