...
public class ElementRules {
	/**
	 * Static text field, one or more of the fields in the rule must have content
	 */
	public static String oneOrMoreMustExist = "oneOrMoreMustExist";
	/**
	 * Static text field, if the first field has content so must all of the other
	 * fields
	 */
	...
	/**
	 * Static text field, if the first field has content then at least one more
	 * field has content also.
	 */
	...
	/**
	 * Static text field, if first has content then no others may have content
	 */
	 ...
	 */
	public ElementRules(String inRule, String inPositions) throws OBOEException {
		ruleText = inRule;
		...
		if (inRule.compareTo(oneOrMoreMustExist) == 0) {
			...
			ruleNumber = 3;
		} else if (inRule.compareTo(ifFirstExistsThenAtLeastOneMoreMustExist) == 0) {
			ruleNumber = 4;
		...
		for (i = 0; i < inPositions.length(); i++) {
			...
			if ((inPositions.charAt(i) < '0') || (inPositions.charAt(i) > '9')) {
				throw new OBOEException(
						"Invalid character in positions, at position " + (i + 1) + " in string " + inPositions);
			}
		}
		if (positionCount == 0) {
			throw new OBOEException("Not enough positions specified in string " + inPositions);
		}
		...
		if (st.countTokens() != positionCount) {
			throw new OBOEException("Based on count there are not enough positions specified in string " + inPositions);
		}
		...
		while (st.hasMoreElements()) {
			...
			try {
				...
				throw new OBOEException("Invalid position " + sPos + " specified string " + inPositions);
			}
			if (positions[i] < 1) {
				for (j = 0; j < i; j++) {
					if (positions[j] == positions[i]) {
						throw new OBOEException(
								"Position " + positions[j] + " specified more than once in string " + inPositions);
					}
				}
			}
			i++;
		...
		if (i != positionCount) {
			throw new OBOEException(
					"Based on tokenizer there is not an exact match for positions specified in string " + inPositions);
		}
	 ...
	 * @param throwError     if true then the error is thrown and application must
	 *                       catch it, otherwise error is reported in returned
	 *                       object
	 * @throws OBOEException thrown if throwError is true
	 * @return String of errors
	 */
	public String testRules(Object elementsToTest[], TemplateSegment parentTemplate, boolean throwError)
		...
		case 1: // one or more must exist
			...
			for (i = 0; i < positionCount; i++) {
				...
				if ((elementsToTest[pos - 1] instanceof String)) {
					testString = getObjectText(parentTemplate, pos - 1, elementsToTest[pos - 1]);
					if (testString.length() > 0) {
						return null;
					}
					if (parentTemplate.isTemplateComposite(pos)) {
						tc = (TemplateCompositeElement) parentTemplate.getTemplateElement(pos);
						sb.append(tc.getShortName() + " ");
					} else {
						tde = (TemplateDataElement) parentTemplate.getTemplateElement(pos);
			...
			if (throwError) {
				...
				return ("rule failure: at least one of these data element fields must be used: " + sb.toString());
			}
		...
		case 2: // if First Exists Then All Must Exist
			...
			for (i = 1; i < positionCount; i++) {
				...
				if (pos > elementsToTest.length) {
					if (throwError) {
						throw new OBOEException(
								"rule failure: if First Exists Then All Must Exist at position " + positions[i]);
					} else {
		...
		case 3: // oneAndOnlyOneMayExist
			...
			cnt = 0;
			if (!throwError) {
				sb = new StringBuilder("enter data into only ONE of these fields: ");
			}
			for (i = 0; i < positionCount; i++) {
				pos = positions[i];
				if (pos > elementsToTest.length) {
			...
			if (cnt > 1) {
				if (throwError) {
					throw new OBOEException("rule failure: one and only one may exist");
				} else {
					return (sb.toString());
				}
			}
		...
		case 4: // ifFirstExistsThenAtLeastOneMoreMustExist
			...
			if (cnt == 0) {
				if (throwError) {
					throw new OBOEException("rule failure: if first exists then at least one more must exist");
				} else {
					return ("if " + name + " is used then use least one of the following " + sb.toString());
				}
			}
			return null;
		...
		case 6: // ifFirstExistsThenNoOthersMayExist
			...
			if (elementsToTest[pos - 1] instanceof String) {
				...
				sb.append("if " + parentTemplate.getTemplateElement(pos).getShortName()
			...
			for (i = 1; i < positionCount; i++) {
				...
				sb.append(parentTemplate.getTemplateElement(pos).getShortName());
				if (throwError) {
					throw new OBOEException("rule failure: if first exists then no others may exist");
				} else {
					return ("rule failure: if first exists then no others may exist" + sb.toString());
				}
			}
			return null;
	 ...
	 * @return String of errors
	 */
	public String testRules(Object elementsToTest[], Segment parentSegment, boolean throwError) throws OBOEException {
		int i;
		DataElement de;
		CompositeElement cde;
		StringBuilder sb = new StringBuilder();
		...
		case 1: // one or more must exist
			...
			for (i = 0; i < positionCount; i++) {
				...
				if ((elementsToTest[pos - 1] instanceof String)) {
					testString = getObjectText(parentSegment, pos - 1, elementsToTest[pos - 1]);
					if (testString.length() > 0) {
						return null;
					}
		...
		case 2: // if First Exists Then All Must Exist
			...
			if (elementsToTest[pos - 1] instanceof CompositeElement) {
				...
				throw new OBOEException("no data element defined at position " + pos);
			}
		...
		case 6: // ifFirstExistsThenNoOthersMayExist
			...
			} else // better be a composite
			{
				...
				cde = (CompositeElement) parentSegment.getElement(pos);
				sb.append("if " + cde.getShortName() + " is used then don't use the following: ");
			}
			for (i = 1; i < positionCount; i++) {
				pos = positions[i];
				if (pos > elementsToTest.length) {
	 ...
	 */
	public static String getObjectText(Object parent, int elementPos, Object inObject) {
