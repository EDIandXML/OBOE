...
public class TransactionSetFactory extends DefaultHandler implements ContentHandler {
	 ...
	 * OBOE.properties file contains: messageDescriptionFolder = xmlDefinitions/
	 * <br>
	 ...
	 * method will search for file named: 004010/P/000001/AAAAA/840.xml <br>
	 * if not found then it will search for file named:
	 * xmlDefinitions/004010/P/000001/840.xml <br>
	 * if not found then it will search for file named:
	 * xmlDefinitions/004010/P/840.xml <br>
	 * if not found then it will search for file named:
	 * xmlDefinitions/004010/840.xml <br>
	 * if not found then it will search for file named: xmlDefinitions/840.xml <br>
	...
	public static TransactionSet buildTransactionSet(String inTSID, String inSearchDirective, String inVersionDirectory,
		...
		if (searchDirective != null) {
			{
				...
				searchLoop: for (int idirs = 0; idirs < lilDirectives.length; idirs++) {
																		...
																		// one
					{
						...
						xmlFile = TransactionSetFactory.class.getClassLoader()
		...
		if (foundXMLFile == false) {
			...
			xmlFile = TransactionSetFactory.class.getClassLoader().getResourceAsStream(messageDescriptionFolderPath);
		}
		...
		try {
			...
			synchronized (tsBuilt) {
				...
				logr.debug("using ts description file" + messageDescriptionFolderPath);
				...
				is = new InputSource(
						TransactionSetFactory.class.getClassLoader().getResourceAsStream(messageDescriptionFolderPath));
