...
/**
 * @author Joe McVerry class for wrapping a ACH messages within an EDI Envelope
 *
 */
public class ACHEnvelope extends Envelope {
	 ...
	 */
	@Override
	public void parse(PushbackInputStream inpis) throws OBOEException, FileNotFoundException, IOException {
		Segment seg;
		int i;
		...
		if (Util.rightTrim(id).compareTo(getMyTemplate().getTemplateSegment(idHeader).getID()) != 0) {
			throw new OBOEException("Expecting " + getMyTemplate().getTemplateSegment(idHeader).getID() + " got " + id);
		}
		...
		while ((getMyTemplate().getTemplateFunctionalGroup().getTemplateSegment(ACHFunctionalGroup.idHeader) != null)
				&& (id.compareTo(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(ACHFunctionalGroup.idHeader).getID()) == 0)) {
			FunctionalGroup functionalGroup = new ACHFunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(),
					this);
			addFunctionalGroup(functionalGroup);
			seg = functionalGroup.createAndAddSegment(ACHFunctionalGroup.idHeader);
			SegmentParser.parse(seg, pis);
			if (pis.read(me) != 1) {
				throw new OBOEException("expected data not read");
			}
			...
			while (id.compareTo("6") == 0) {
				...
				TransactionSet parsedTransactionSet = TransactionSetFactory.buildTransactionSet("ACH");
			...
			// pis.unread(me);
			seg = functionalGroup.createAndAddSegment(ACHFunctionalGroup.idTrailer);
			SegmentParser.parse(seg, pis);
		...
		if (id.compareTo(getMyTemplate().getTemplateSegment(idTrailer).getID()) != 0) {
			throw new OBOEException(
					"Expecting " + getMyTemplate().getTemplateSegment(idTrailer).getID() + " got " + id);
		}
	...
	public void resetBatchHeader() { // reset fg batch header record, if
		// necessary
		TemplateFunctionalGroup tfg = getMyTemplate().getTemplateFunctionalGroup();
	...
	public void rebuildBatchHeader() { // rebuild fg batch header record
		TemplateFunctionalGroup tfg = getMyTemplate().getTemplateFunctionalGroup();
		TemplateSegment tseg = tfg.getTemplateSegment(ACHFunctionalGroup.idHeader);
		tseg.setID("+");
		tseg = tfg.getTemplateSegment("-");
		tseg.setID(ACHFunctionalGroup.idHeader);
	...
	public String getFormattedText(Format format) throws OBOEException {
		...
		if (format == Format.CSV_FORMAT) {
			...
			sb.append("<envelope format=\""
					+ ((transID.equals("CBR")) || (transID.equals("PBR")) ? "ACH-CBR-PBR" : "ACH") + "\"");
	...
	public void writeFormattedText(Writer inWriter, Format format) throws OBOEException, IOException {
		...
		if (format == Format.CSV_FORMAT) {
			...
			if ((transID.equals("CBR")) || (transID.equals("PBR"))) {
				inWriter.write("<envelope format=\"ACH-CBR-PBR\"" + io.github.ediandxml.oboe.util.Util.lineFeed);
			} else {
				inWriter.write("<envelope format=\"ACH\"" + io.github.ediandxml.oboe.util.Util.lineFeed);
			}
	...
	public FunctionalGroup createFunctionalGroup() {
		return new ACHFunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(), this);
	}
