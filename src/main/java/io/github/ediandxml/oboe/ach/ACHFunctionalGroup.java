...
public class ACHFunctionalGroup extends FunctionalGroup {
	...
	public ACHFunctionalGroup(TemplateFunctionalGroup inTFG, IContainedObject inParent) {
	...
	public void setCountInTrailer() throws OBOEException {
		...
		getTrailer().setDataElementValue("28", getHeader().getElement("28").get());

		String l = Integer.toString(getTransactionSetCount());
		StringBuilder sb = new StringBuilder();
		int testL = ((DataElement) getTrailer().getElement("97")).getMinLength() - l.length();
	...
	public void validate(DocumentErrors inDErr) {
		...
		if (header != null) {
			...
			inDErr.addError(0, "Envelope", "Missing FunctionalGroup Header", this, "3", this, ERROR_TYPE.Integrity);
		}

		int i;
		TransactionSet ts;
		if (getTransactionSetCount() == 0) {
			inDErr.addError(0, "FG", "No transaction sets", this, "4", this, ERROR_TYPE.Integrity);
		} else {
			for (i = 0; i < getTransactionSetCount(); i++) {
				ts = getTransactionSet(i);
				ts.validate(inDErr);
		...
		if (getTransactionSetCount() < 1) {
			inDErr.addError(0, "FG", "No Transaction Sets Defined", this, "4", this, ERROR_TYPE.Integrity);
		}

		if (trailer != null) {
			trailer.validate(inDErr);
		} else {
			inDErr.addError(0, "FG", "Missing FunctionalGroup Trailer", this, "4", this, ERROR_TYPE.Integrity);
		}
