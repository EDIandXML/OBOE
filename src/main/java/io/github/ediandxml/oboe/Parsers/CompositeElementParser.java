...
public class CompositeElementParser {
	 ...
	 * @throws OBOEException most likely the composite item can't find any of the
	 *                       fields coming in.
	 * @return boolean true if tokenized string was used
	 */

	public static boolean parse(CompositeElement compositeElement, String parseThisString,
			ITokenizer transactionSetTokenizer) throws OBOEException {

		DataElement currentDE;

		DataTokenizer dataTokenizer = new DataTokenizer(parseThisString, transactionSetTokenizer.getElementSeparator(),
				transactionSetTokenizer.getEscapeCharacters());

		int elementsToParse = dataTokenizer.countTokens();

		var allTempElements = ((TemplateCompositeElement) compositeElement.myTemplate).getAllTemplateElements();

		var cnt = 0;

		var $token = Util.unEscape(dataTokenizer.nextToken(), transactionSetTokenizer.getEscapeCharacters());
		while (elementsToParse > 0) {
			cnt++;
			if ($token.length() > 0) {
				if (!allTempElements.containsKey(cnt)) {
					throw new OBOEException("No data element defined at position " + cnt + " for composite "
			...
			currentDE.set($token);
			elementsToParse--;
			$token = Util.unEscape(dataTokenizer.nextToken(), transactionSetTokenizer.getEscapeCharacters());
	...
	public static void parse(CompositeElement compositeElement, PushbackInputStream pis)
		...
		for (i = 1; i <= template.getContainerSize(); i++) {
			...
			if (template.isTemplateComposite(i + 1)) {
				throw new OBOEException("Can't have composite element within a composite element.");
			} else if (template.isTemplateDE(i)) {
				TemplateDataElement tde = (TemplateDataElement) template.getTemplateElement(i);
	...
	public static void parse(CompositeElement compositeElement, Node node) throws OBOEException {
		...
		nodeLoop: for (n = 0; n < nl.getLength(); n++) {
			...
			if (compositeElement.getTemplate() != null) {
				i = compositeElement.getTemplate().doYouUseThisXMLElement(currentSubNodeName, currentPosition);
			} else {
				i = compositeElement.doIUseThisXMLElement(currentSubNodeName, currentPosition);
			}

			currentPosition = i;
			var tc = (TemplateCompositeElement) compositeElement.myTemplate;
			if (tc.myElementContainer.templateDataElementList.get(i).IAmATemplateComposite()) {
				throw new OBOEException("Can't have composite element within a composite element.");
			}

			currentDE = (DataElement) compositeElement.getElement(currentPosition);
			if (currentDE == null) {
				currentDE = (DataElement) compositeElement.buildElement(currentPosition);
			}
