...
public class SegmentParser {
	/**
	 * parses an EDI Document adds to datalement ArrayList and adds to secondary
	 * segment ArrayList
	 *
	 *
	...
	public static boolean parse(Segment segment, ITokenizer transactionSetTokenizer) throws OBOEException {
		...
		if ((segment.myTemplate != null) && (elementsToParse > segment.myTemplate.getLastPosition())) {
			transactionSetTokenizer.reportError("Too many data elements " + elementsToParse
					+ " is greater than template count of " + segment.getTemplate().getContainerSize()
					+ "; at byte offset(" + transactionSetTokenizer.getInputByteCount() + ") near segment ["
					+ transactionSetTokenizer.getSegmentPos() + "]");
		} else if ((segment.myTemplate == null) && (elementsToParse > segment.myTemplate.getLastPosition())) {
			transactionSetTokenizer.reportError("Too many data elements " + elementsToParse
					+ " is greater than built segment count of " + segment.getContainerSize() + "; at byte offset("
					+ transactionSetTokenizer.getInputByteCount() + ")near segment ["
					+ transactionSetTokenizer.getSegmentPos() + "]");
			return false;
		}
		...
		/* && */ transactionSetTokenizer.hasMoreDataElements()) {
			String currentTokenString = transactionSetTokenizer.getCurrentDataElement();
			...
			if (segment.isCompositeElement(cnt)) {
				...
				if (currentComposite.getOccurs() > 1) {
					DataTokenizer dataTokenizer = new DataTokenizer(currentTokenString,
							transactionSetTokenizer.getRepeaterCharacter(),
							transactionSetTokenizer.getEscapeCharacters());
					for (int dti = 0; dti < dataTokenizer.countTokens(); dti++) {
						if (dti > 0) {
							currentComposite.createNewGroup();
						}
						CompositeElementParser.parse(currentComposite, dataTokenizer.getTokenAt(dti),
								transactionSetTokenizer);
					}

				} else {
					if (CompositeElementParser.parse(currentComposite, transactionSetTokenizer.getCurrentDataElement(),
				...
				try {
					if (currentDE.getOccurs() > 1) {
						...
						if (dataTokenizer.countTokens() == 0) {
							...
							for (int dti = 0; dti < dataTokenizer.countTokens(); dti++) {
								currentDE.setNext(dataTokenizer.getTokenAt(dti));
							}
						}
					} else {
						currentDE.set(Util.unEscape(transactionSetTokenizer.getCurrentDataElement(),
								transactionSetTokenizer.getEscapeCharacters()));
					}
				} catch (Exception exc) {
					transactionSetTokenizer.reportError("Parsing Error: " + exc.getMessage() + "; at byte offset("
							+ transactionSetTokenizer.getInputByteCount() + ")");
				}

			} else if (currentTokenString.length() > 0) {
				transactionSetTokenizer.reportError("Parsing Error: element (\"" + currentTokenString
						+ "\") at position " + (cnt) + " is not used in segment " + segment.getID()
						+ "; at byte offset(" + transactionSetTokenizer.getInputByteCount() + ")near segment ["
						+ transactionSetTokenizer.getSegmentPos() + "]");
			}
			transactionSetTokenizer.getNextDataElement();
		...
		if (transactionSetTokenizer.getNextSegment((MetaContainer) segment.getParent()) == null) {
			return true;
		}
	...
	/**
	 * parses a XML EDI Document adds to datalement ArrayList or CompositeElement
	 * and adds to secondary segment ArrayList
	 *
	...
	public static int parse(Segment segment, Node node) throws OBOEException {
		...
		nodeLoop: for (n = 0; n < nl.getLength(); n++) {
			...
			if ((lastSubNodeName != null) && lastSubNodeName.equals(currentSubNodeName)) {

				if (segment.isCompositeElement(currentPosition)) {
					if (currentComposite.getOccurs() != 1) {
						currentComposite.createNewGroup();
						CompositeElementParser.parse(currentComposite, currentNode);
						continue nodeLoop;
					}
				} else {
						...
						for (int nn = 0; nn < cnl.getLength(); nn++) {
							...
							if (cnode.getNodeType() == Node.TEXT_NODE) {
								...
								if (currentDE instanceof NumericDE) {
									((NumericDE) currentDE).setFormatted(cnode.getNodeValue()); // why
																								// not
																								// set
																								// next?
								} else {
			...
			if (segment.myTemplate != null) {
				currentPosition = segment.myTemplate.doYouUseThisXMLElement(currentSubNodeName, currentPosition);
			} else {
				currentPosition = segment.doIUseThisXMLElement(currentSubNodeName, currentPosition);
			}
			...
			if (currentPosition >= 0) {
				...
				if (segment.isCompositeElement(currentPosition)) {
					currentComposite = (CompositeElement) segment.getElement(currentPosition);
					if (currentComposite.getShortName().compareTo(currentSubNodeName) == 0) {
						CompositeElementParser.parse(currentComposite, currentNode);
				...
				{
					currentDE = (DataElement) segment.getElement(currentPosition);
	...
	public static boolean parse(Segment segment, PushbackInputStream pis) throws OBOEException, IOException {
		int i = 0;
		...
		for (i = 1; i <= segment.myTemplate.getContainerSize(); i++) {
			...
			if (segment.myTemplate.isTemplateComposite(i + 1)) {
				...
				TemplateDataElement tde = (TemplateDataElement) segment.myTemplate.getTemplateElement(i);
