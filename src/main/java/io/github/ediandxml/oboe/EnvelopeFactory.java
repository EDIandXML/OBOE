...
/**
 * class for building an Envelope from an edi xml file this is a subclass of the
 * SAX2 handler <br>
...
public class EnvelopeFactory extends DefaultHandler implements ContentHandler {
	 ...
	 */
	protected String xmlDirectoryPath = "." + System.getProperty("file.separator") + ".";
	/**
	 ...
	 * process logic
	 * <ul>
	 * <li>test each name found for edi type: Envelope, table, segment, data element
	...
	public void startElement(java.lang.String uri, java.lang.String localName, java.lang.String rawName,
		...
		if (name.equals("segment")) {
			...
			currentTemplateSegment = new TemplateSegment(currentID, currentName, sequence, description, occurs,
					required, shortName, true, currentContainer);

			if (validatingMethod != null) {
				currentTemplateSegment.setValidatingMethod(
						TransactionSetFactory.getValidatingMethod(validatingMethod, currentTemplateSegment));
			}
		...
		if (name.equals("compositeDE")) {
			currentCompositeElement = setTemplateComposite(currentTemplateSegment.getContainerSize(), attributes);
			try {
		...
		if (name.equals("idListFile")) {
			...
			currentIDList = idLists.get(xmlFoundDirectoryPath + attributes.getValue("fileName"));
			if (currentIDList == null) {

				currentIDList = new IDList(xmlFoundDirectoryPath + attributes.getValue("fileName"), xmlDirectoryPath,
						idListParser);
				idLists.put(xmlFoundDirectoryPath + attributes.getValue("fileName"), currentIDList);
		...
		throw new SAXException(
				"logic error: unknown type " + name + ". Last item was " + currentID + " at element # " + _iElement);
	...
	public TemplateCompositeElement setTemplateComposite(int pos, Attributes attributes) throws SAXException {
		...
		TemplateCompositeElement currentTemplateComposite = new TemplateCompositeElement(id, thisName, required,
				position, description, shortName, currentTemplateSegment, occurs, true);
	...
	public TemplateDataElement setDataElement(int pos, Attributes attributes) throws SAXException {
		...
		TemplateDataElement currentTemplateDE = new TemplateDataElement(id, thisName, position, type,
				required.charAt(0), description, minLength, maxLength, shortName, null, null, occurs, true);
	...
	public void warning(SAXParseException e) throws SAXException {
		logr.error("Warning at (file " + (knownSystemID == null ? e.getSystemId() : knownSystemID) + ", line "
	...
	public void error(SAXParseException e) throws SAXException {
		throw new SAXException("Error at (file " + (knownSystemID == null ? e.getSystemId() : knownSystemID) + ", line "
				+ e.getLineNumber() + ", char " + e.getColumnNumber() + "): " + e.getMessage());
	}
	...
	public void fatalError(SAXParseException e) throws SAXException {
		throw new SAXException("Fatal Error at (file " + (knownSystemID == null ? e.getSystemId() : knownSystemID)
	 ...
	 * OBOE.properties definition see OBOE.properties file to define the directory
	 * path
	 *
	 * @param inFileName String without the path or xml file extension
	 * @param version    String specifies subpath to find version specific envelope
	...
	public static TemplateEnvelope buildEnvelope(String inFileName, String version) {
		...
		try {
			...
			logr.debug(
					" searching for envelope xml in classpath as: " + msgDescriptionSearchPath + inFileName + ".xml");
			is = new InputSource(EnvelopeFactory.class.getClassLoader()
					.getResourceAsStream(msgDescriptionSearchPath + inFileName + ".xml"));
			if (is.getByteStream() == null) {
				logr.debug(
						" envelope xml not found in classpath as: " + msgDescriptionSearchPath + inFileName + ".xml");
				logr.debug(" searching for envelope xml in classpath as: " + path + inFileName + ".xml");
				is = new InputSource(
						EnvelopeFactory.class.getClassLoader().getResourceAsStream(path + inFileName + ".xml"));
				if (is.getByteStream() == null) {
					throw new OBOEException("Envelope Message Description (" + version + inFileName
							+ ".xml) not found in classpath as specified in OBOE.properties file.)");
				}
				msgDescriptionSearchPath = path;
			}
			logr.debug(" using envelope xml in classpath as: " + path + inFileName + ".xml");
