...
/**
 * Manages errors in XML documents for the OBOE (Open Business Objects for EDI)
 * framework. Tracks and reports errors related to XML parsing and validation.
 */
public class XMLErrors {
	...
	private record ErrorRecord(String description, IContainedObject container, Node node, Integer lineNumber,
			Integer level, Boolean reportable) {
	}
	...
	/**
	 * Constructs an XMLErrors instance, initializing the error level from OBOE
	 * properties.
	 */
	public XMLErrors() {
		try {
	...
	public void setErrorLevel(int level) {
		if (level < 1 || level > 10) {
			logger.error("Error level out of range (1-10). Value passed: {}", level);
			throw new IllegalArgumentException("Error level must be between 1 and 10");
		}
	 ...
	 */
	public void addError(String description, IContainedObject container, Node node, int level) {
		if (level <= errorLevel) {
			errorRecords.add(new ErrorRecord(description, container, node, null, level, false));
		}
	}
	 ...
	 * @param level       The error level
	 */
	public void addError(int lineNumber, String description, IContainedObject container, int level) {
		if (level <= errorLevel) {
			errorRecords.add(new ErrorRecord(description, container, null, lineNumber, level, false));
		}
	}
	...
	/**
	 * Gets the line number associated with the error at the specified index.
	 *
	 * @param index The index in the error records list
	...
	private String formatErrorMessage(int index) {
		...
		if (container == null) {
			message.append("Error at: ").append(getLineNumber(index)).append(" Description: ")
					.append(getErrorDescription(index));
		} else {
			message.append("Segment position: ").append(getLineNumber(index)).append(" Description: ")
	...
	private String formatLogMessage(int index) {
		StringBuilder message = new StringBuilder(getErrorDescription(index)).append(" ");
		IContainedObject container = getContainer(index);
