 ...
 * create an OBOE object. The application reads in a message description to
 * generate the code. A lot of code is generated but not enough to be completely
 * useful. You will have to remove code for segments and data elements you don't
 * use and add code for logic that is specific to your system requirements.
 * format: java OBOECodeGenerator p|bJ envelope tsid [-useName] [outputFileName]
 * where p generates a Java program that parses an EDI document or b generates a
 * Java program that builds an EDI document and envelope is the envelope xml
 * file to use and tsid is the TransactionSet id to build and optional -useName
...

{
		...
		if (table != null) {
			...
			for (var elms : table.getContainer()) {
				...
				if (elms.getContainerType() == ContainerType.Loop) {
					...
					if (segment.isUsed()) {
						if (useName) {
							pw.write("		buildSegment" + segment.getShortName() + "forTableHeader(table);"
									+ lineFeed);
						} else {
		...
		if (table != null) {
			...
			for (var elms : table.getContainer()) {
				...
				if (elms.getContainerType() == ContainerType.Loop) {
					...
					if (segment.isUsed()) {
						if (useName) {
							pw.write("		buildSegment" + segment.getShortName() + "forTableDetail(table);"
									+ lineFeed);
						} else {
		...
		if (table != null) {
			...
			for (var elms : table.getContainer()) {
				...
				if (elms.getContainerType() == ContainerType.Loop) {
					...
					if (segment.isUsed()) {
						if (useName) {
							pw.write("		buildSegment" + segment.getShortName() + "forTableSummary(table);"
									+ lineFeed);
						} else {
	...
	private String buildSegmentCodeForJava(Object parent, String name, TemplateSegment seg, String methodName) {
		...
		if (parent instanceof TemplateTable) {
			if (useName) {
				pw.write("public Segment buildSegment" + seg.getShortName() + "for" + name + "(Table inTable)"
						+ lineFeed + "  throws OBOEException" + lineFeed + "{" + lineFeed);
			} else {
