...
public class IDListEditor extends JFrame implements ActionListener, WindowListener, ChangeListener {
	 ...
	 */
	public IDListEditor(TransactionSetMessageEditor inParent, IDList idList, TemplateDataElement edited)
			throws Exception {
	...
	public boolean save() {
		...
		((IDList) workingDE.getIDList()).setName("IDList" + workingDE.getID() + ".xml");
		...
		try {
			...
			for (int r = 0; r < idViewer.getRowCount(); r++) {
				...
				if (b.booleanValue()) {
					idlsave.add((String) idViewer.getValueAt(r, 1), (String) idViewer.getValueAt(r, 2));
				}
				if (b.booleanValue() == include) {
					...
					if (Util.isInteger(sb2str) && Util.isInteger(lastV)
							&& (Integer.parseInt(sb2str) < Integer.parseInt(lastV))) {
						sb2.append('-' + lastV);
			...
			e1.printStackTrace();
			JOptionPane.showMessageDialog(this,
					"Unknown Java Excetion - see stack trace in console window for " + e1.getMessage(), e1.getMessage(),
