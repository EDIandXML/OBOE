...
public class TradacomsEnvelope extends Envelope {
	 ...
	 * STX and END and creates one emtpy functional group
	 *
	...
	public void parse(Node node) throws OBOEException {
		...
		if (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
				.getTemplateSegment(TradacomsFunctionalGroup.idHeader).getShortName()) != 0) {

			FunctionalGroup functionalGroup = new TradacomsFunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(),
					this);
			...
			while (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
				...
				addFunctionalGroup(functionalGroup);
				seg = new Segment(functionalGroup.getTemplateSegment(TradacomsFunctionalGroup.idHeader),
						functionalGroup);
				SegmentParser.parse(seg, cnode);
				...
				while (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(TradacomsFunctionalGroup.idTrailer).getShortName()) != 0) {
				...
				seg = new Segment(functionalGroup.getTemplateSegment(TradacomsFunctionalGroup.idTrailer),
						functionalGroup);
				SegmentParser.parse(seg, cnode);
				functionalGroup.addSegment(seg);
				for (i++; i < nl.getLength(); i++) {
	...
	public void parse(PushbackInputStream pis) throws OBOEException, FileNotFoundException, IOException {
		...
		if (id.compareTo(getMyTemplate().getTemplateFunctionalGroup()
				.getTemplateSegment(TradacomsFunctionalGroup.idHeader).getID()) != 0) {
			...
			while (id.compareTo(getMyTemplate().getTemplateSegment(TradacomsFunctionalGroup.idHeader).getID()) == 0) {
				FunctionalGroup functionalGroup = new TradacomsFunctionalGroup(
						getMyTemplate().getTemplateFunctionalGroup(), this);
				addFunctionalGroup(functionalGroup);
				seg = new Segment(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(TradacomsFunctionalGroup.idHeader), functionalGroup);
				SegmentParser.parse(seg, pis);
				if (pis.read(me) != 4) {
					throw new OBOEException("expected data not read");
				}
				...
				while (id.compareTo(
						getMyTemplate().getTemplateSegment(TradacomsFunctionalGroup.idTrailer).getID()) != 0) {
				...
				seg = new Segment(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(TradacomsFunctionalGroup.idTrailer), functionalGroup);
				SegmentParser.parse(seg, pis);
				if (pis.read(me) != 4) {
					throw new OBOEException("expected data not read");
				}
	...
	public FunctionalGroup createFunctionalGroup() {
		return new TradacomsFunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(), this);
	}
	...
	@Override
	public void writeFormattedText(Writer inWriter, Format format) throws OBOEException, IOException {

		if (format == Format.VALID_XML_FORMAT) {
			inWriter.write("<?xml version=\"1.0\"?>" + io.github.ediandxml.oboe.util.Util.lineFeed);

			inWriter.write("<envelope format=\"Tradacoms\"" + io.github.ediandxml.oboe.util.Util.lineFeed);
			inWriter.write("  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\""
					+ io.github.ediandxml.oboe.util.Util.lineFeed);
			inWriter.write(
		...
		if (format == Format.XML_FORMAT) {
			inWriter.write("<?xml version=\"1.0\"?>" + io.github.ediandxml.oboe.util.Util.lineFeed);
			inWriter.write("<Envelope>" + io.github.ediandxml.oboe.util.Util.lineFeed);
		}
	...
	public Segment createInterchange_Header() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(TradacomsEnvelope.idInterchangeHeader);
	...
	public Segment createInterchange_Trailer() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(TradacomsEnvelope.idInterchangeTrailer);
		Segment seg = new Segment(tsg, this);
		addSegment(seg);
		return seg;
	}
