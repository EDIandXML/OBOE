...
public class TradacomsDocumentHandler implements EDIDocumentHandler {
	 ...
	 * @param inReader the edi document in a java io reader object
	 * @exception OBOEException - unknown transaction set, this transaction set is
	 *                          undefined to OBOE - parsing erros
	 */
	...
	public void startSegment(Segment inSeg) {
		...
		if (inSeg.getID().compareTo(TradacomsEnvelope.idInterchangeHeader) == 0) {
			envelope.addSegment(inSeg);
		} else if (inSeg.getID().compareTo(TradacomsFunctionalGroup.idHeader) == 0) {
			functionalGroup.addSegment(inSeg);
		} else if (inSeg.getID().compareTo(TradacomsFunctionalGroup.idTrailer) == 0) {
			functionalGroup.addSegment(inSeg);
		} else if (inSeg.getID().compareTo(TradacomsEnvelope.idInterchangeTrailer) == 0) {
			envelope.addSegment(inSeg);
		}
	...
	/**
	 * main method used for testing purposes format: java filetoparse writes valid
	 * xml format of file to System.out
	 *
	...
	public static void main(String args[]) {
		...
		for (String arg : args) {
			try {
				...
				x.writeFormattedText(new PrintWriter(System.out), Format.VALID_XML_FORMAT);
