 ...
 * Document handlers will register with this class to be notified when specific
 * edi objects are created or finished Unlike the old parser these parsers will
 * not contain the objects, the process of adding objects to owning parents
 * (such as adding functional groups to an envelope) is left up to the document
 * handler.
 */
...
public class TradacomsDocumentParser extends EDIDocumentParser {
	...
	public boolean parseDocument(String inString) throws OBOEException {
		...
		if (posStart < 0) {
			throw new OBOEException(TradacomsEnvelope.idInterchangeHeader + " segment missing");
		}
		int posStop = inString.indexOf(TradacomsEnvelope.idInterchangeTrailer);
		if (posStop < 0) {
			throw new OBOEException(TradacomsEnvelope.idInterchangeTrailer + " segment missing");
		}
	 ...
	 * @return boolean - true - continue or false - halted if false is used then
	 *         don't forget to call the validation method on the envelope object
	 *         pass it the documenterrors object created here and getable using the
	 *         getDocumentErrors method.
	 *
	 * @exception OBOEException - most likely unknown segment
	 */
	...
	public boolean parseDocument(Reader inReader, boolean inValidate) throws OBOEException {
		...
		TemplateEnvelope te = EnvelopeFactory.buildEnvelope("Tradacoms.envelope", "notSetYet");
		...
		if (findID.compareTo(TradacomsEnvelope.idInterchangeHeader) != 0) {
			throw new OBOEException(
					"Segment ID " + findID + " found, was expecting " + TradacomsEnvelope.idInterchangeHeader + ".");
		}
		...
		boolean iterateOnce = (findID.compareTo(TradacomsFunctionalGroup.idHeader) != 0);
		if (iterateOnce) {
		...
		while (findID.compareTo(TradacomsFunctionalGroup.idHeader) == 0) {
			...
			if (iterateOnce) {
				...
				Segment BAT_Functional_Group_Header = fg.createAndAddSegment(TradacomsFunctionalGroup.idHeader);
				BAT_Functional_Group_Header.setByteOffset(et.getInputByteCount());
			...
			while (findID.equals("MHD")) {
				...
				et.resetSegment();
				DataElement de = (DataElement) ((CompositeElement) STX_Start_of_Transmission.getElement("FROM"))
						.getElement(2);
				String from = "";
				if (de != null) {
					from = de.get();
				}
				de = (DataElement) ((CompositeElement) STX_Start_of_Transmission.getElement("UNTO")).getElement(2);
				String unto = "";
				if (de != null) {
					unto = de.get();
				}
				...
				parsedTransactionSet = TransactionSetFactory.buildTransactionSet(findID, null,
						...
						((CompositeElement) STX_Start_of_Transmission.getElement("STDS")).getElement(1).get(), // version
						from, // receiver
						unto, // sender
				...
				while (true) {
					...
					if (findID.equals("MHD")) {
						...
						et.reportError("Should not appears before end of functional group", "2");
						notifyEndTransactionSet(parsedTransactionSet);
						if (halted()) {
							return false;
						}
						...
						et.getNextDataElement();
						parsedTransactionSet.continueParse(et.getLastMetaContainer(), et);
					} else {
			...
			if (iterateOnce) {
				...
				dErr.addError(0, TradacomsFunctionalGroup.idTrailer,
						"More segments to process starting at segment position " + et.getSegmentPos() + ". Problem "
								+ et.getCurrentDataElement() + " is not defined. Expecting "
								+ TradacomsFunctionalGroup.idTrailer,
						envelope, "0", null, ERROR_TYPE.Integrity);
				/*
				 * throw new OBOEException( "More segments to process starting at position " +
				 * et.getSegmentPos() + ". Problem " + et.getCurrentDataElement() +
				 * " is not defined. Expecting " + TradacomsFunctionalGroup.idTrailer);
				 */
			} else {
				Segment UNE_Functional_Group_Trailer = fg.createAndAddSegment(TradacomsFunctionalGroup.idTrailer);
				UNE_Functional_Group_Trailer.setByteOffset(et.getInputByteCount());
		...
		if (findID.compareTo(TradacomsEnvelope.idInterchangeTrailer) != 0) {
			...
			/*
			 * throw new OBOEException(
			 * "More segments to process starting at segment position " + et.getSegmentPos()
			 * + ". Problem " + et.getCurrentDataElement() + " is not defined. Expecting " +
			 * TradacomsEnvelope.idInterchangeTrailer);
			 */
		} else {
			Segment END_Interchange_Trailer = envelope.createInterchange_Trailer();
