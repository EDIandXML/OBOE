...
public class X12FunctionalGroup extends FunctionalGroup {
	...
	/**
	 * instantiates a functional group from the definition in an envelope message
	 * description file.
	 */

	public X12FunctionalGroup(TemplateFunctionalGroup inTFG, IContainedObject inParent) {
		super(inTFG, inParent, idHeader, idTrailer);

	}

	/**
	 * set the Transaction Count in the trailer object also sets the trailer "28"
	 * field from the headers "28" field
	 *
	...
	public boolean validate() throws OBOEException {
		...
		for (i = 0; i < getTransactionSetCount(); i++) {
			...
			if (fggrp.compareTo(ts.getFunctionalGroup()) != 0) {
				throw new OBOEException("Functional Identifier Code (GS01-479) value is " + fggrp
						+ " does not match for Transaction Set ID " + ts.getID() + "-" + ts.getFunctionalGroup());
			}
			ts.validate();
		}
		...
		if ((header != null) && (trailer != null)) {
			...
			if (saidCount != readCount) {
				throw new OBOEException(
						"Transaction Set Count Mismatch. Said " + saidCount + ".  Should Be " + readCount);
			}
		}
		return true;
	}
	...
	/**
	 * validates doesn't throw exception but placess error message in DocumentErrors
	 * object
	 */
	...
	public void validate(DocumentErrors inDErr) {
		...
		if (header != null) {
			...
			inDErr.addError(0, "Envelope", "Missing FunctionalGroup Header", this, "3", this,
		...
		if (getTransactionSetCount() == 0) {
			inDErr.addError(0, "FG", "No transaction sets", this, "4", this, DocumentErrors.ERROR_TYPE.Integrity);
		} else {
			for (i = 0; i < getTransactionSetCount(); i++) {
				ts = getTransactionSet(i);
				if (fggrp.compareTo(ts.getFunctionalGroup()) != 0) {
					inDErr.addError(0, "FG", "Functional Identifier Code (GS01-479) value is " + fggrp
							+ " does not match for Transaction Set ID " + ts.getID() + "-" + ts.getFunctionalGroup(),
							this, "6", this, DocumentErrors.ERROR_TYPE.Requirement);
				}
				ts.validate(inDErr);
			}
		}
		...
		if (getTransactionSetCount() < 1) {
			inDErr.addError(0, "FG", "No Transaction Sets Defined", this, "4", this,
					DocumentErrors.ERROR_TYPE.Integrity);
		}
		...
		if (trailer != null) {
			...
			inDErr.addError(0, "FG", "Missing FunctionalGroup Trailer", this, "4", this,
					DocumentErrors.ERROR_TYPE.Integrity);
		}
		...
		if ((header != null) && (trailer != null) && (hNoErr == true) && (tNoErr == true)) {
			...
			if ((de1 != null) && (de2 != null)) {
				if ((de1.get() != null) && (de2.get() != null)) {
					if (de1.get().compareTo(de2.get()) != 0) {
						inDErr.addError(0, "FG", "Control number mismatch (28)", this, "4", header,
								DocumentErrors.ERROR_TYPE.Integrity);
					}
				}
			}
			...
			if (saidCount != readCount) {
				inDErr.addError(0, "97", "Transaction Set Count Mismatch.  Should Be " + readCount, this, "7",
						trailer.getElement("97"), DocumentErrors.ERROR_TYPE.Integrity);
			}
	...
	public Segment buildHeaderSegment() {
		...
		Segment seg = new Segment(getMyTemplate().getTemplateSegment(idHeader), this);
		addContainer(seg);
		return seg;
	}
	...
	public Segment buildTrailerSegment() {
		Segment seg = new Segment(getMyTemplate().getTemplateSegment(idTrailer), this);
		addContainer(seg);
		return seg;
	}
