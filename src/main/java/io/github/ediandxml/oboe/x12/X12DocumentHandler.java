...
public class X12DocumentHandler implements EDIDocumentHandler {
	...
	/**
	 * create a parser for transaction set and parser what is coming from Reader
	 * object
	 */
	...
	/**
	 * create a parser for transaction set and parser what is coming from Reader
	 * object if you use this constructor and there are document errors the method
	 ...
	 *                          undefined to OBOE - parsing errors
	 */
	...
	/**
	 * called when an Segment object is created only called for segments at the
	 * Envelope and functionalGroup level does not get called for segments within
	 * TransactionSet
	 * 
	...
	public void startSegment(Segment inSeg) {
		...
		} else if (inSeg.getID().compareTo(X12Envelope.idInterchangeTrailer) == 0) {
			envelope.addSegment(inSeg);
		} else if (inSeg.getID().compareTo(X12Envelope.idGradeofServiceRequest) == 0) {
			envelope.addSegment(inSeg);
		} else if (inSeg.getID().compareTo(X12Envelope.idDeferredDeliveryRequest) == 0) {
			envelope.addSegment(inSeg);
		} else if (inSeg.getID().compareTo(X12Envelope.idInterchangeAcknowledgment) == 0) {
			envelope.addSegment(inSeg);
	...
	/**
	 * called when an Segment object is finished only called for segments at the
	 * Envelope and functionalGroup level does not get called for segments within
	 * TransactionSet
	 * 
	 * @param inSeg Segment found
	 ...
	 * contains an X12 message args[1] String - optional integer value for output
	 * format XML_FORMAT = 1; X12_FORMAT = 2; EDIFACT_FORMAT = 3 not logical
	 * VALID_XML_FORMAT = 4; CSV_FORMAT = 5; FIXED_LENGTH_FORMAT = 6;
	 * TRADACOMS_FORMAT = 7 not logical VALID_XML_FORMAT_WITH_POSITION = 8;
	 * ACH_FORMAT = 9 not logical ACH_FORMAT_FOR_CBR_PBR = 10 not logical must
	 * contain an x12 edi document
	 */
	...
	public static void main(String args[]) {
		...
		try {
			...
			env.writeFormattedText(new PrintWriter(System.out), Format.VALID_XML_FORMAT);
