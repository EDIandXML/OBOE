...
public class X12Envelope extends Envelope {
	...
	/**
	 * instantiates the class from a TemplateEnvelope, creates mandatory segments
	 * ISA and IEA and creates one emtpy functional group
	 *
	...
	public void parse(Node node) throws OBOEException {
		...
		if (cnode.getNodeName()
				.compareTo(getMyTemplate().getTemplateSegment(idInterchangeHeader).getShortName()) != 0) {
			throw new OBOEException(
					"Expecting " + getMyTemplate().getTemplateSegment(idInterchangeHeader).getShortName());
		}
		...
		if (gd.compareTo(Envelope.X12_GROUP_DELIMITER) != 0) {
			...
			String t = Envelope.X12_SEGMENT_DELIMITER + Envelope.X12_FIELD_DELIMITER + gd
					+ Envelope.X12_REPEAT_DELIMITER + Envelope.X12_ESCAPE_CHARACTER;
		...
		if ((getMyTemplate().getTemplateSegment(idGradeofServiceRequest) != null) && (cnode.getNodeName()
				.compareTo(getMyTemplate().getTemplateSegment(idGradeofServiceRequest).getShortName()) == 0)) {
		...
		if ((getMyTemplate().getTemplateSegment(idDeferredDeliveryRequest) != null) && (cnode.getNodeName()
				.compareTo(getMyTemplate().getTemplateSegment(idGradeofServiceRequest).getShortName()) == 0)) {
			seg = new Segment(getMyTemplate().getTemplateSegment(idDeferredDeliveryRequest), this);
			addSegment(seg);
			SegmentParser.parse(seg, cnode);
			for (i++; i < nl.getLength(); i++) {
				cnode = nl.item(i);
		...
		if ((getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment) != null) && (cnode.getNodeName()
				.compareTo(getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment).getShortName()) == 0)) {

			do {
				seg = new Segment(getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment), this);
				addSegment(seg);
				SegmentParser.parse(seg, cnode);
				for (i++; i < nl.getLength(); i++) {
					cnode = nl.item(i);
		...
		while ((getMyTemplate().getTemplateFunctionalGroup().getTemplateSegment(X12FunctionalGroup.idHeader) != null)
				&& (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(X12FunctionalGroup.idHeader).getShortName()) == 0)) {
			FunctionalGroup functionalGroup = new X12FunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(),
					this);
			...
			while ((getMyTemplate().getTemplateFunctionalGroup()
					...
					&& (cnode.getNodeName().compareTo(getMyTemplate().getTemplateFunctionalGroup()
							.getTemplateSegment(X12FunctionalGroup.idTrailer).getShortName()) != 0)) {
				String tsID = null;
				...
				if ((tsID == null) || (tsID.length() < 1)) {
					throw new OBOEException("Can't find TransactionSet id value in " + cnode.getNodeName());
				}

				TransactionSet parsedTransactionSet = TransactionSetFactory.buildTransactionSet(tsID, null,
						functionalGroup.getSegment(X12FunctionalGroup.idHeader).getElement("480").get(),
						getSegment(X12Envelope.idInterchangeHeader).getElement("I07").get(),
						getSegment(X12Envelope.idInterchangeHeader).getElement("I06").get(),
						getSegment(X12Envelope.idInterchangeHeader).getElement("I14").get());
		...
		if (cnode.getNodeName()
				.compareTo(getMyTemplate().getTemplateSegment(idInterchangeTrailer).getShortName()) != 0) {
			throw new OBOEException(
					"Expecting " + getMyTemplate().getTemplateSegment(idInterchangeTrailer).getShortName());
		}

		seg = new Segment(getMyTemplate().getTemplateSegment(idInterchangeTrailer), this);
		SegmentParser.parse(seg, cnode);
		addSegment(seg);
	...
	@Override
	public void parse(PushbackInputStream pis) throws OBOEException, FileNotFoundException, IOException {
		...
		if (id.compareTo(getMyTemplate().getTemplateSegment(idInterchangeHeader).getID()) != 0) {
			throw new OBOEException(
					"Expecting " + getMyTemplate().getTemplateSegment(idInterchangeHeader).getID() + " got " + id);
		}

		seg = new Segment(getMyTemplate().getTemplateSegment(idInterchangeHeader), this);
		addSegment(seg);
		...
		if ((getMyTemplate().getTemplateSegment(idGradeofServiceRequest) != null)
				&& (id.compareTo(getMyTemplate().getTemplateSegment(idGradeofServiceRequest).getID()) == 0)) {
			seg = new Segment(getMyTemplate().getTemplateSegment(idGradeofServiceRequest), this);
			addSegment(seg);
			SegmentParser.parse(seg, pis);
			if (pis.read(me) != 2) {
		...
		if ((getMyTemplate().getTemplateSegment(idDeferredDeliveryRequest) != null)
				&& (id.compareTo(getMyTemplate().getTemplateSegment(idGradeofServiceRequest).getID()) == 0)) {
			seg = new Segment(getMyTemplate().getTemplateSegment(idDeferredDeliveryRequest), this);
			addSegment(seg);
			SegmentParser.parse(seg, pis);
		...
		if ((getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment) != null)
				&& (id.compareTo(getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment).getID()) == 0)) {
			...
			} while (id.compareTo(getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment).getID()) == 0);
		}
		...
		while ((getMyTemplate().getTemplateFunctionalGroup().getTemplateSegment(X12FunctionalGroup.idHeader) != null)
				&& (id.compareTo(getMyTemplate().getTemplateFunctionalGroup()
						.getTemplateSegment(X12FunctionalGroup.idHeader).getID()) == 0)) {
			FunctionalGroup functionalGroup = new X12FunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(),
					this);
			addFunctionalGroup(functionalGroup);
			seg = new Segment(getMyTemplate().getTemplateSegment(X12FunctionalGroup.idHeader), functionalGroup);
			functionalGroup.addSegment(seg);
		...
		if (id.compareTo(getMyTemplate().getTemplateSegment(idInterchangeTrailer).getID()) != 0) {
			throw new OBOEException(
					"Expecting " + getMyTemplate().getTemplateSegment(idInterchangeTrailer).getID() + " got " + id);
		}
	...
	public FunctionalGroup createFunctionalGroup() {
		return new X12FunctionalGroup(getMyTemplate().getTemplateFunctionalGroup(), this);
	}
	...
	/**
	 * gets the count of Interchange_Acknowledgment in the ArrayList(container)
	 *
	 * @return int count
	 */
	...
	/**
	 * gets a Interchange_Acknowledgment from the ArrayList(container) check for
	 * runtime array out of bounds exception
	 *
	...
	/**
	 * creates, sets and returns the Interchange Control Header built for the
	 * envelope
	 *
	 * @return Segment - can return null if templateenvelope is null or segment not
	 *         defined in envelope message description file
	 */
	...
	public Segment createInterchange_Header() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(idInterchangeHeader);
		if (tsg == null) {
			return null;
		}
	...
	public Segment createGrade_of_Service_Request() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(idGradeofServiceRequest);
		if (tsg == null) {
			return null;
		}
		Segment seg = new Segment(tsg, this);
	...
	public Segment createDeferred_Delivery_Request() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(idDeferredDeliveryRequest);
		if (tsg == null) {
			return null;
		}
		Segment seg = new Segment(tsg, this);
	...
	public Segment createInterchange_Acknowledgment() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(idInterchangeAcknowledgment);
		if (tsg == null) {
			return null;
		}
		Segment seg = new Segment(tsg, this);
	...
	/**
	 * creates, sets and returns the Interchange Control Trailer built for the
	 * envelope
	 *
	 * @return Segment - can return null if templateenvelope is null or segment not
	 *         defined in envelope message description file
	...
	public Segment createInterchange_Trailer() {
		...
		TemplateSegment tsg = getMyTemplate().getTemplateSegment(idInterchangeTrailer);
		if (tsg == null) {
			return null;
		}
		Segment seg = new Segment(tsg, this);
	...
	public void setDelimitersInHeader() throws OBOEException {
		...
		if ((repeatDelimiter.charAt(0) != '\u0000')
				&& (repeatDelimiter.equals(Envelope.PREBUILD_REPEAT_DELIMITER) == false)) {
	...
	/**
	 * sets the functional group count in the trailer. also sets the control number
	 * from header
	 */
	...
	public void setFGCountInTrailer() throws OBOEException {
		...
		getInterchange_Trailer().setDataElementValue("I12", getInterchange_Header().getElement("I12").get());

		getInterchange_Trailer().setDataElementValue("I16", Integer.toString(getFunctionalGroupCount()));
	}
	...
	public String getFormattedText(Format inFormat) {
		...
		if (format == Format.PREBUILD_FORMAT) {
			...
			if (repeatDelimiter.charAt(0) != '\u0000') {
				seg.setDataElementValue("I10", Envelope.PREBUILD_REPEAT_DELIMITER);
			}
		...
		if (seg == null) {
			sb.append(io.github.ediandxml.oboe.util.Util.lineFeed + "missing IEA segment"
					+ io.github.ediandxml.oboe.util.Util.lineFeed);
		} else {
			sb.append(seg.getFormattedText(format));
		}
	...
	@Override
	public void writeFormattedText(Writer inWriter, Format inFormat) throws OBOEException, IOException {
		...
		if (format == Format.CSV_FORMAT) {
			writer.write("Envelope" + io.github.ediandxml.oboe.util.Util.lineFeed);
		} else if ((format == Format.VALID_XML_FORMAT) || (format == Format.VALID_XML_FORMAT_WITH_POSITION)) {
			writer.write("<?xml version=\"1.0\"?>" + io.github.ediandxml.oboe.util.Util.lineFeed);
			writer.write("<envelope format=\"X12\"" + io.github.ediandxml.oboe.util.Util.lineFeed);
		...
		seg = getSegment(idInterchangeTrailer);
		if (seg == null) {
			writer.write(io.github.ediandxml.oboe.util.Util.lineFeed + "missing IEA segment"
					+ io.github.ediandxml.oboe.util.Util.lineFeed);
		} else {
			writer.write(seg.getFormattedText(format));
		}
	...
	/**
	 * validate contents of envelope doesn't throw exception but places errors in
	...
	public void validate(DocumentErrors inDErr) {
		...
		seg = getSegment(idInterchangeHeader);
		if (seg == null) {
			inDErr.addError(0, "Envelope", "Missing ISA Segment", this, "1", this, DocumentErrors.ERROR_TYPE.Integrity);
		} else {
		...
		FunctionalGroup fg;
		if ((getFunctionalGroupCount() == 0) && (ta1Found == false)) {
			inDErr.addError(0, "Envelope", "No functional groups", this, "3", this,
		...
		seg = getSegment(idInterchangeTrailer);
		if (seg == null) {
			inDErr.addError(0, "Envelope", "Missing IEA Segment", this, "3", this, DocumentErrors.ERROR_TYPE.Integrity);
		} else {
			...
			if ((hNoErr == true) && (tNoErr == true) && (seghdr != null)) {
				...
				if ((de1 != null) && (de2 != null)) {
					if ((de1.get() != null) && (de2.get() != null)) {
						if (de1.get().compareTo(de2.get()) != 0) {
							inDErr.addError(0, idInterchangeHeader, "Control number mismatch (I12)", this, "3", seghdr,
									DocumentErrors.ERROR_TYPE.Integrity);
						}
					}
