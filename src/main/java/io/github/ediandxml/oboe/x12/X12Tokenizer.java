...
/**
 * class to assist in tokenizing input transaction sets x12 field separator uses
 * 3rd byte of input string x12 segment separator uses 16th field + 1 byte
 * field, if it sees a cr character then checks for a lf character and then
 * assumes a \\n character EDIFACT uses different control positions. see header
 * segment.
 * 
...
public class X12Tokenizer extends Tokenizer implements ITokenizer {
	 ...
	 */
	public X12Tokenizer(Reader inReader, DocumentErrors inDErr) throws OBOEException {
		...
		for (i = 0; i < 107; i++) {
			if (firstSeg[i] == firstSeg[3]) {
				...
				if (tokencnt == 16) {
					if (i > 103) {
						if (inDErr != null) {
							inDErr.addError(0, "Envelope", "ISA segment too long, unable to continue", null, "E", null,
					...
					if (i < 103) {
						if (inDErr != null) {
							inDErr.addError(0, "Envelope", "ISA segment too short, attempting to continue", null, "E",
									null, DocumentErrors.ERROR_TYPE.Integrity);
						}
						logr.error("ISA segment too short, attempting to continue");
					}

					if ((firstSeg[i + 1] == 10) || (firstSeg[i + 1] == 13) || (firstSeg[i + 1] == '\n')) {
						if (inDErr != null) {
							inDErr.addError(0, "Envelope", "ISA segment in error, missing component element separator",
									null, "E", null, DocumentErrors.ERROR_TYPE.Integrity);
						} else {
		...
		/*
		 * tokenGroups[0] = firstSeg[104]; tokenseparatorCharacter = firstSeg[105]+"";
		 */

		// TransactionSetTokenizer = new DataTokenizer(str,
		// tokenseparatorCharacter, escapeCharacters);
		transactionSetTokenizer = new ReaderTokenizer(pbr, tokenseparatorCharacter, escapeCharacters);
