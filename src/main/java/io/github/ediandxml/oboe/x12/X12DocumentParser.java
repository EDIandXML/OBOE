...
public class X12DocumentParser extends EDIDocumentParser {
	 ...
	 * @exception OBOEException - unknown transaction set, this transaction set is
	 *                          undefined to OBOE
	 */
	@Override
	public boolean parseDocument(String inString) throws OBOEException {
		...
		if (posStart < 0) {
			throw new OBOEException(X12Envelope.idInterchangeHeader + " segment missing");
		}
		int posStop = inString.indexOf(X12Envelope.idInterchangeTrailer);
		if (posStop < 0) {
			throw new OBOEException(X12Envelope.idInterchangeTrailer + " segment missing");
		}
	 ...
	 * @param inValidate - if true call validation method of the envelope after
	 *                   parsing
	 * @return boolean - true - continue or false - halted if you pass false don't
	 *         forget to call the validation method on the envelope object pass to
	 *         the validation method the documenterrors object created here, use the
	 *         getDocumentErrors method of the parser object.
	 * @exception OBOEException - unknown transaction set, this transaction set is
	 *                          undefined to OBOE
	 */

	@Override
	public boolean parseDocument(Reader inReader, boolean inValidate) throws OBOEException {
		...
		if (findID.compareTo(X12Envelope.idInterchangeHeader) != 0) {
			throw new OBOEException(
					"Segment ID \"" + findID + "\" found, was expecting " + X12Envelope.idInterchangeHeader + "\"");
		}

		Segment ISA_Interchange_Control_Header = envelope.createInterchange_Header();
		ISA_Interchange_Control_Header.setByteOffset(tokenizer.getInputByteCount());
		// should be 0
		...
		if (findID.compareTo(X12Envelope.idGradeofServiceRequest) == 0) {
			...
			ISB_Grade_of_Service_Request.setByteOffset(tokenizer.getInputByteCount());
		...
		if (findID.compareTo(X12Envelope.idDeferredDeliveryRequest) == 0) {
			...
			ISE_Deferred_Delivery_Request.setByteOffset(tokenizer.getInputByteCount());
		...
		if (findID.compareTo(X12Envelope.idInterchangeAcknowledgment) == 0) {
			...
			do {
				Segment Temp_Interchange_Acknowledgment = envelope.createInterchange_Acknowledgment();
				Temp_Interchange_Acknowledgment.setByteOffset(tokenizer.getInputByteCount());
		...
		if (findID.compareTo(X12FunctionalGroup.idHeader) != 0) {
			if (!ta1Found) {
				throw new OBOEException(
						"Segment ID \"" + findID + "\" found, was expecting \"" + X12FunctionalGroup.idHeader + "\"");
			}
		}
		...
		while ((findID != null) && (findID.compareTo(X12FunctionalGroup.idHeader) == 0)) {
			...
			Segment GS_Functional_Group_Header = functionalGroup.createAndAddSegment(X12FunctionalGroup.idHeader);
			GS_Functional_Group_Header.setByteOffset(tokenizer.getInputByteCount());
			...
			while ((findID != null) && (findID.equals("ST"))) {
				...
				String getImplementationConventionReference = tokenizer.getDataElementAt(3);
				if (getImplementationConventionReference == null) {
					getImplementationConventionReference = GS_Functional_Group_Header.getElement("480").get();
				} else {
					// it's an error if they don't match
					if (getImplementationConventionReference
							.equals(GS_Functional_Group_Header.getElement("480").get())) {
						;
					} else {
						dErr.addError(tokenizer.getSegmentPos(), "ST", "GS08 and ST03 do not match", functionalGroup,
								"?", tokenizer, DocumentErrors.ERROR_TYPE.Integrity);
					}
				...
				parsedTransactionSet = TransactionSetFactory.buildTransactionSet(findID, null,
						...
						// or
						// GS09
						ISA_Interchange_Control_Header.getElement("I07").get(), // receiver
						ISA_Interchange_Control_Header.getElement("I06").get(), // sender
						ISA_Interchange_Control_Header.getElement("I14").get()); // test
				...
				while (true) {
					...
					if (findID.equals("ST")) {
						...
						tokenizer.reportError("Should not appear before end of functional group", "2");
						notifyEndTransactionSet(parsedTransactionSet);
						if (this.halted()) {
							return false;
						}
						...
						// sync.
						tokenizer.getNextSegment(tokenizer.getLastMetaContainer());
						tokenizer.getNextDataElement();
						if (parsedTransactionSet.continueParse(tokenizer.getLastMetaContainer(), tokenizer) == false) {
							tokenizer.reportError("May not be able to restart parser", "?");
							// break;
						}
					} else {
			...
			if (findID == null) {
				...
				tokenizer.reportError("Unknown or out of place segment (" + findID + ")", "2");

			} else {
				Segment GE_Functional_Group_Trailer = functionalGroup.createAndAddSegment(X12FunctionalGroup.idTrailer);
				GE_Functional_Group_Trailer.setByteOffset(tokenizer.getInputByteCount());
		...
		if (findID == null) {
			...
			IEA_Interchange_Control_Trailer.setByteOffset(tokenizer.getInputByteCount());
		...
		if (io.github.ediandxml.oboe.util.Util.propertyFileIndicatesTHROW_PARSING_EXCEPTION()
