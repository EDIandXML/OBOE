...
public class RulesAndCode_ID extends JFrame
	...
	RulesAndCodeBuilder parent;
	IDList idl;

	String filterList = "";
	boolean include = true;
	IDListParser idlp;
	static Logger logr = LogManager.getLogger(RulesAndCode_ID.class.getName());
	 ...
	 */
	public RulesAndCode_ID(RulesAndCodeBuilder inParent,
			TemplateDataElement edited) throws Exception {

		super(edited.getName());
		idlp = new IDListParser();

		parent = inParent;
		workingDE = edited;
		try {
			String filename = Util.getMessageDescriptionFolder() + "IDList"
					+ edited.getID() + ".xml";

			idl = new IDList(filename, Util.getMessageDescriptionFolder(),
					idlp);
		} catch (Exception e) {
