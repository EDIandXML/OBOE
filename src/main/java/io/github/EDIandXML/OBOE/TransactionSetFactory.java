...
public class TransactionSetFactory extends DefaultHandler
		...
		if (nodeName.equals("elementRule")) {
			...
			for (int ci = 1; ci <= currentRule.getPositionCount(); ci++) {
				if ((currentRule.getPosition(ci) < 1) || (currentRule
						.getPosition(ci) > currentSegment.getContainerSize())) {
					throw new SAXException(
	...
	public static TransactionSet buildTransactionSet(String inTSID)
		...
		if (Util.doWeFindRulesFilesInClassPath()) {
			return buildTransactionSetByClasspath(inTSID, null, null, null,
					null, null, true);
		} else {
			return buildTransactionSet(inTSID, null, null, null, null, null,
					true);
		}
	...
	/**
	 * static class method will build a transaction set FROM A message
	 * description LOCATED IN A CLASSPATH JAR FILE based on input string,
	 * the searchDirective, the four search directories and OBOE.properties
	 * definition see OBOE.properties file to define the directory path and
	 * optional searchDirective. At the very least the message description
	 * must reside in the directory specified by the xmlPath. The message
	 * description name is appended with ".xml" <br>
	 * example #1 no search directive <br>
	 * OBOE.properties file contains: xmlPath = c:/xmlDefinitions/ <br>
	 * input String is 840 <br>
	 * method will read file named: c:/xmlDefinitions/840.xml <br>
	 * example #2 full search directive <br>
	 * OBOE.properties file contains: xmlPath = c:/xmlDefinitions/ <br>
	 ...
	 *                                    specified by the next four
	 *                                    parameters. The base directory is
	 *                                    defined by the xmlPath property in
	 *                                    OBOE.properties. Any combination
	 *                                    is possible to provide for the
	 *                                    search directive. The directive
	 *                                    value of "STV" - indicates to
	 *                                    search using the SendId parameter
	 *                                    value as a directory, Test
	 *                                    Indicator as as Directory, and
	 *                                    Version parameter value as a
	 *                                    directory. While "VR" indicates to
	 *                                    use the Version number as a
	 *                                    directory and Receiver id. The
	 *                                    package starts at the lowest
	 *                                    directory with its search. If the
	 *                                    message description files is not
	 *                                    found there it will go up to the
	 *                                    next directory and repeat this
	 *                                    process until the directory as
	 *                                    specified by the xmlPath property.
	 *                                    <br>
	 *                                    <i><b>Note</b></i>If a null is
	 *                                    passed, the method will look in
	 *                                    the OBOE.properties file for a
	 ...
	 * @throws OBOEException io exception
	 */
	public static TransactionSet buildTransactionSetByClasspath(String inTSID,
			String inSearchDirective, String inVersionDirectory,
			String inReceiverIDDirectory, String inSenderIDDirectory,
			String inTestOrProductionDirectory,
			boolean inSaveInArrayListIndicator) {

		// xmlPath - OBOE.properties value
		// xmlSearchPath - directory path to found message description file
		// xmlFilePath - full path name to message description file

		String xmlPath = "", xmlSearchPath = "", xmlFilePath = "";
		...
		String searchPaths[] = null;
		if ((searchDirective == null)
				|| (searchDirective.trim().length() == 0)) {
			xmlFilePath = xmlPath + inTSID + ".xml";
			is = EnvelopeFactory.class.getClassLoader()
					.getResourceAsStream(xmlFilePath);
			if (is != null) {
				xmlSearchPath = xmlPath;
				foundXMLFile = true;
			}
		} else if (searchDirective != null) {

			String lilDirectives[] = searchDirective.split(",");
			searchLoop: for (int idirs = 0; idirs < lilDirectives.length; idirs++) {

				searchPaths = new String[lilDirectives[idirs].length() + 1];
				StringBuilder searchPath = new StringBuilder(xmlPath);
				...
				for (i = 0; i < lilDirectives[idirs].length(); i++) {
					...
					case 'T':
						if ((inTestOrProductionDirectory != null)
							...
							searchPath
									.append(inTestOrProductionDirectory.trim());
							searchPath.append(File.separatorChar);
						}
				...
				for (i = lilDirectives[idirs].length(); i >= 0; i--)
				// there's at least one
				{
					xmlFilePath = searchPaths[i] + inTSID + ".xml";

					logr.debug("looking for " + inTSID + " in classpath as "
							+ xmlFilePath);

					is = TransactionSetFactory.class.getClassLoader()
							.getResourceAsStream(xmlFilePath);
					if (is != null) {
		...
		if (inSaveInArrayListIndicator) {
			TemplateTransactionSet tts = tsBuilt.get(xmlFilePath);
			if (tts != null) {
				return new TransactionSet(tts, null);
			}
		}
		...
		if (foundXMLFile == false) {
			throw new OBOEException(
					"XML Message Description not found in classpath: "
							+ xmlFilePath);
		}
		...
		try {
			...
			synchronized (tsBuilt) {
				...
				logr.debug("using ts xml classpath file: " + xmlFilePath);
