...
/**
 * Id List Processor to be used with the X12 data element id = 103. aka Packing
 * Code
 *
...
public class IDList103Processor implements IDListProcessor {
	...
	static Logger logr = LogManager.getLogger(IDList103Processor.class.getName());
	...
	public IDList103Processor() throws IOException {
		...
		if (Util.doWeFindRulesFilesInClassPath()) {
			msgDescriptionFolder = msgDescriptionFolder.substring("%classpath%".length());
		}
		IDList currentIDList = new IDList("testFiles/IDList103Part1.xml", msgDescriptionFolder, idListParser);
		...
		currentIDList = new IDList("testFiles/IDList103Part2.xml", msgDescriptionFolder, idListParser);
